...
class FadingChannel(Generic[Array]):
    ...
    Example:
        >>> from gpu3gppchan import TdlChannelConfig, FadingChannel
        >>>
    ...
    def __init__(
        ...
        *,
        ...
        # Create internal _native carrier config
        ...
        # Convert channel config to _native format
        self._native_cfg = channel_config._to_native(
        ...
        # (TdlConfig and CdlConfig from nanobind both have these attributes)
        self._native_cfg.signal_length_per_ant = n_symbol_slot * n_sc  # type: ignore[union-attr]
        self._native_cfg.batch_len = [n_sc] * n_symbol_slot  # type: ignore[union-attr]
        self._native_cfg.proc_sig_freq = 1  # type: ignore[union-attr]

        # Create channel using new constructor without tx_signal_in
        if self._is_tdl:
            self._channel = _native.TdlChan(
                tdl_cfg=self._native_cfg,
                rand_seed=channel_config.rand_seed,
                stream_handle=self._stream
            )
        else:
            self._channel = _native.CdlChan(
                cdl_cfg=self._native_cfg,
                rand_seed=channel_config.rand_seed,
                stream_handle=self._stream
            )
        ...
        if not disable_noise:
            self._noise_adder = _native.GauNoiseAdder(
                num_threads=1024,
                rand_seed=channel_config.rand_seed,
                stream_handle=self._stream
            )
    ...
    ) -> _native.CarrierParams:  # type: ignore[name-defined]
        """Create internal _native carrier config."""
        cfg = _native.CarrierParams()
        cfg.n_sc = n_sc
        cfg.mu = numerology
        cfg.n_fft = n_fft
    ...
    def run(
        ...
        *,
        ...
        input_stream: Optional[CudaStream] = None,
    ) -> Array:
        ...
        Args:
            ...
            input_stream: Stream that produced a CuPy input. When omitted, the
                current CuPy stream is synchronized before native execution.
        ...
        # Wrap CuPy array into _native type for C++ binding
        freq_in_cuda = _native.CudaArrayComplexFloat(freq_in_gpu)
        ...
        # The returned array may next be consumed on CuPy's default stream.
        # Explicitly complete work submitted to this non-blocking stream first;
        # the legacy blocking stream provided this ordering implicitly.
