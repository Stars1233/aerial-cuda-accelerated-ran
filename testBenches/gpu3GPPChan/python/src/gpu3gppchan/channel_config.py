...
"""gpu3gppchan — channel models configuration.
...
class FadingChannelConfig(ABC):
    ...
    ) -> Union[_native.TdlConfig, _native.CdlConfig]:  # type: ignore[name-defined]
        """Convert to _native config object. Must be implemented by subclasses."""
        pass
...
class TdlChannelConfig(FadingChannelConfig):
    ...
    def _to_native(
        ...
        carrier_n_sc_prbg: int, carrier_sc_spacing_hz: float,
        carrier_n_symbol_slot: int
    ) -> _native.TdlConfig:  # type: ignore[name-defined]
        """Convert to _native.TdlConfig for internal use."""
        cfg = _native.TdlConfig()
...
class CdlChannelConfig(FadingChannelConfig):
    ...
    def _to_native(
        ...
        carrier_n_sc_prbg: int, carrier_sc_spacing_hz: float,
        carrier_n_symbol_slot: int
    ) -> _native.CdlConfig:  # type: ignore[name-defined]
        """Convert to _native.CdlConfig for internal use."""
        cfg = _native.CdlConfig()
        cfg.delay_profile = self.delay_profile.upper()
        cfg.delay_spread = self.delay_spread
        cfg.max_doppler_shift = self.max_doppler_shift
        cfg.n_cell = self.n_cell
...
# re-exported from gpu3gppchan. See nanobind docstrings
