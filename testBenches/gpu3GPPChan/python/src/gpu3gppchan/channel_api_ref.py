...
"""
gpu3gppchan library - channel models API.

DOCUMENTATION ONLY - This file is kept for reference and discussion. Runtime types
come from ``gpu3gppchan._gpu3gppchan`` (nanobind); ``aerial.phy5g.channel_models`` re-exports
the same names. **Attribute names below are the Python ``snake_case`` names** you
set on binding objects or pass into constructors—not C++ struct field names.

Primary import for applications::

    from gpu3gppchan import (
        StatisticalChannel,
        SimConfig, SystemLevelConfig, LinkLevelConfig, ExternalConfig,
        CellParam, UtParamCfg, AntPanelConfig, Coordinate,
        StParam, SpstParam, Scenario, SensingTargetType, UeType,
    )

Low-level wrapper (same configs):: ``import gpu3gppchan._gpu3gppchan as _native`` then
``_native.StatisChanModel(...)``.
"""
...
#
# Stubs mirror ``_native`` / 3GPP TR 38.901: system-level (UMa, UMi, RMa),
...
#
# Usage: align these names with live ``_native`` objects and YAML where
...
# Binding quirks and defaults: see nanobind docstrings for your SDK revision.
...
class Scenario:
    """Deployment scenario per 3GPP TR 38.901 (Python: ``_native.Scenario`` enum).
...
class SensingTargetType:
    """ISAC sensing target kind (Python: ``_native.SensingTargetType`` enum).
...
class UeType:
    """UE equipment category (Python: ``_native.UeType`` enum).
...
class Coordinate:
    ...
    Python binding: ``_native.Coordinate`` with float fields.
...
class SpstParam:
    """One scattering point on a sensing target (Python: ``_native.SpstParam``).
    ...
    exposed in every binding—check ``_native`` for extra properties.)
...
class StParam:
    """One sensing target for ISAC (Python: ``_native.StParam``).
...
class AntPanelConfig:
    """Antenna panel definition (Python: ``_native.AntPanelConfig``).
...
class UtParamCfg:
    """Per-UE configuration (Python: ``_native.UtParamCfg``).
...
class CellParam:
    """Per-sector base station cell (Python: ``_native.CellParam``).
...
class SystemLevelConfig:
    ...
    Python: ``_native.SystemLevelConfig``. Paired with ``SimConfig`` (waveform /
    ...
    **UT dropping**
    ...
    def __init__(self,  # pylint: disable=too-many-arguments,too-many-locals
                 ...
                 # -1 = auto (Table 7.4.1-1). Order matches _native binding.
                 ...
                 st_size_ind: int = 0,  # Sensing-target size class:
...
class LinkLevelConfig:
    """Small-scale model inside the statistical channel (Python: ``_native.LinkLevelConfig``).
...
class SimConfig:
    """Waveform / OFDM / output tensor policy (Python: ``_native.SimConfig``).
    ...
    **Python attributes**
        ...
        run_mode — int: **0** CIR taps only; **1** CIR+CFR(PRBG); **2** CIR+CFR(SC);
            **3** CIR+CFR both granularities; **4** CIR+CFR on full **N_FFT** grid
            (no PRBG aggregation). (Short labels in nanobind docstrings may list a
        ...
        internal_memory_mode — **0** outputs only to caller-provided tensors on
            ...
            In modes 1/2, copy internally stored results with ``get_cir()`` and,
            for mode 2, ``get_cfr()`` into caller-provided output tensors.
...
class ExternalConfig:
    """Static topology for SLS (Python: ``_native.ExternalConfig``).
...
class StatisChanModel:
    """C++ stochastic channel backend exposed as ``_native.StatisChanModel``.
    ...
    CuPy conversion for output buffers supplied to ``run()``, ``get_cir()``,
    and ``get_cfr()``.
    ...
    **Methods (typical binding surface)**
        ...
        ``get_cir`` / ``get_cfr`` — copy internally stored channel data into
            caller-provided buffers after ``run()``.
    """
    ...
    def get_cir(self,
        ...
        """Copy internally stored CIR into caller-provided buffers."""
        pass

    def get_cfr(self,
                cfr_sc: Optional[List[np.ndarray]] = None,
                cfr_prbg: Optional[List[np.ndarray]] = None) -> None:
        """Copy internally stored CFR into caller-provided buffers."""
        pass

    def dump_los_nlos_stats(self,
                            los_nlos_stats: Optional[np.ndarray] = None
                            ) -> None:
        """
