...
static __global__ void ofdmMod_ifft_kernel(ofdmModDynDescr_t<Tscalar, Tcomplex> * ofdmModdynDescpr)
{
    ...
    uint32_t N_sc_over_2 = ofdmModdynDescpr -> N_sc >> 1; // divide by 2
    uint32_t N_IFFT = ofdmModdynDescpr -> N_IFFT;
    ...
    const uint32_t freq_offset = ofdmModdynDescpr -> N_sc * global_fft_id;
    const uint32_t CP_current = ofdmModdynDescpr -> cpInfo[blockIdx.y * FFT::ffts_per_block + threadIdx.y]; // CP length for current OFDM symbol
    const uint32_t CP_offset  = ofdmModdynDescpr -> cpInfo[blockIdx.y * FFT::ffts_per_block + threadIdx.y + (ofdmModdynDescpr -> N_symbol_slot)]; // CP_offset in this layer
    const uint32_t time_offset = cufftdx::size_of<FFT>::value * global_fft_id + CP_offset + (ofdmModdynDescpr -> cpInfo[(ofdmModdynDescpr -> N_symbol_slot) * 2 - 1]) * blockIdx.x; // FFT size + CP offset in this layer + CP offset in previous layers
...
static __global__ void applyWindow(ofdmModDynDescr_t<Tscalar, Tcomplex> * ofdmModdynDescpr)
{
    ...
    uint32_t N_IFFT = ofdmModdynDescpr -> N_IFFT;
    const uint32_t ofdmSymId = threadIdx.y;
    const uint32_t CP_current = ofdmModdynDescpr -> cpInfo[ofdmSymId]; // CP length for current OFDM symbol
    const uint32_t CP_offset  = ofdmModdynDescpr -> cpInfo[ofdmSymId + (ofdmModdynDescpr -> N_symbol_slot)]; // CP_offset in this layer
    const uint32_t block_offset = N_IFFT*ofdmSymId + CP_offset + (N_IFFT * (ofdmModdynDescpr -> N_symbol_slot) + ofdmModdynDescpr -> cpInfo[(ofdmModdynDescpr -> N_symbol_slot) * 2 - 1]) * blockIdx.x; // FFT size + CP offset in this layer + CP offset in previous layers

    /*---------    Apply windowing  ------------ */
    if(ofdmModdynDescpr -> ofdmWindowLen > 3) // window resources are configured for lengths > 3
    {
        uint32_t index = block_offset + threadIdx.x;
...
ifftKernelHandle<Tscalar, Tcomplex> ofdmMod_get_ifft_param(dim3& block_dim, uint32_t& shared_memory_size)
{ 
    using namespace cufftdx;
  ...
  */
template<typename Tscalar, typename Tcomplex>
ifftKernelHandle<Tscalar, Tcomplex> ofdmMod_get_ifft_param(const int Nifft, uint32_t cudaDeviceArch, dim3& block_dim, uint32_t& shared_memory_size)
{ 
...
m_enableSwapTxRx(0)
{
    ...
    // NOT USED FOR NOW
    uint32_t ofdmWindowLen = cuphyCarrierPrms -> ofdmWindowLen;
...
ofdmModulate<Tscalar, Tcomplex>::~ofdmModulate()
{
    ...
    if(m_ofdmModdynDescprCpu -> ofdmWindowLen > 3)
    {
