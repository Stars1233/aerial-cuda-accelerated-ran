...
static __global__ void ofdmDeMod_fft_kernel(ofdmDeModDynDescr_t<Tscalar, Tcomplex> * ofdmDeModdynDescpr)
{
    ...
    uint32_t N_sc_over_2 = ofdmDeModdynDescpr -> N_sc >> 1; // divide by 2
    uint32_t N_FFT = ofdmDeModdynDescpr -> N_FFT;
    ...
    // const uint32_t global_fft_id = (blockIdx.x * FFT::ffts_per_block) + local_fft_id;
    const uint32_t global_fft_id = (blockIdx.x * gridDim.y + blockIdx.y) * FFT::ffts_per_block + local_fft_id;
    ...
    const uint32_t freq_offset = ofdmDeModdynDescpr -> N_sc * global_fft_id;
    const uint32_t CP_current = ofdmDeModdynDescpr -> cpInfo[blockIdx.y * FFT::ffts_per_block + threadIdx.y]; // CP length for current OFDM symbol
    const uint32_t CP_offset  = ofdmDeModdynDescpr -> cpInfo[blockIdx.y * FFT::ffts_per_block + threadIdx.y + (ofdmDeModdynDescpr -> N_symbol_slot)]; // CP_offset in this layer
    const uint32_t time_offset = cufftdx::size_of<FFT>::value * global_fft_id + CP_offset + (ofdmDeModdynDescpr -> cpInfo[(ofdmDeModdynDescpr -> N_symbol_slot) * 2 - 1]) * blockIdx.x; // FFT size + CP offset in this layer + CP offset in previous layers
    const uint32_t stride = FFT::stride;
    uint32_t       index  = threadIdx.x + time_offset;
...
static __global__ void ofdmDeMod_fft_prach_kernel(ofdmDeModDynDescr_t<Tscalar, Tcomplex> * ofdmDeModdynDescpr)
{
    ...
    uint32_t N_FFT                      = ofdmDeModdynDescpr->Nfft_RA;
    ...
    // calculate read data positions
    const uint32_t CP_offset            = cpLenPrach;

    // repitition index in this layer
    const uint32_t repIdxThisLayer      = blockIdx.y * FFT::ffts_per_block + local_fft_id;

    // Rx antenna index
    const uint32_t rxAntIdx             = blockIdx.x;

    // The final block can contain a padded FFT when Nrep is not divisible by
    // FFT::ffts_per_block. Its lanes must still participate in the batched FFT,
    // but must not read or write signal buffers. Use the unpadded repetition
    // count for the output stride between RX antennas.
    const bool activeRepetition         = repIdxThisLayer < Nrep;
    const uint32_t global_fft_id        = rxAntIdx * Nrep + repIdxThisLayer;
    ...
    const uint32_t time_offset          = N_samp_slot * rxAntIdx + preambleSampStart + CP_offset + repIdxThisLayer * cufftdx::size_of<FFT>::value;
...
#pragma unroll
    ...
    const uint32_t freq_offset          = Nsamp_oran * global_fft_id;

    const uint32_t loadFftOutStart      = startSC % N_FFT;
    const uint32_t loadFftOutend        = (startSC+L_RA-1) % N_FFT;
    const uint32_t Nsamp_right          = Nsamp_oran - L_RA - kBar;

    if (activeRepetition && index == 0) {
...
#pragma unroll
    ...
    {
        if (activeRepetition &&
            (i * stride + threadIdx.x) < cufftdx::size_of<FFT>::value)
        {
...
fftKernelHandle<Tscalar, Tcomplex> ofdmDeMod_get_fft_param(dim3& block_dim, uint32_t& shared_memory_size, const bool prach)
{ 
    using namespace cufftdx;
  ...
  */
template<typename Tscalar, typename Tcomplex>
fftKernelHandle<Tscalar, Tcomplex> ofdmDeMod_get_fft_param(const int Nfft, uint32_t cudaDeviceArch, dim3& block_dim, uint32_t& shared_memory_size, const bool prach)
{ 
    ...
    m_enableSwapTxRx(0)
{
    ...
    uint32_t N_symbol_slot = cuphyCarrierPrms -> N_symbol_slot;
    ...
    if (m_prach) {// for PRACH
        ...
        uint32_t symbol0IdxPerSubFrame = (cuphyCarrierPrms -> id_slot) * N_symbol_slot;
        ...
        if(cuphyCarrierPrms -> cpType == 0) // normal CP length
        {
            ...
            uint32_t lenCP1 = ((512 >> mu) << (cuphyCarrierPrms -> kappa_bits))*T_c_over_T_samp;
            for(uint8_t symbolIdx = 0; symbolIdx < N_symbol_slot; symbolIdx++)
            {
                m_cpInfoCpu[symbolIdx] = lenCP1;
            }
        ...
        // copy CP info
        CUdeviceptr cpInfoGpu{};
        CHECK_CURESULT(cuMemAlloc(&cpInfoGpu, sizeof(uint16_t) * cpInfoLen));
        m_cpInfoGpu = reinterpret_cast<uint16_t*>(cpInfoGpu);
        m_ofdmDeModdynDescprCpu -> cpInfo = m_cpInfoGpu;
        CHECK_CURESULT(cuMemcpyHtoD(
            reinterpret_cast<CUdeviceptr>(m_cpInfoGpu),
            m_cpInfoCpu,
            sizeof(uint16_t) * cpInfoLen));
    ...
    CUdeviceptr ofdmDeModdynDescprGpu{};
    CHECK_CURESULT(cuMemAlloc(
        &ofdmDeModdynDescprGpu, sizeof(ofdmDeModDynDescr_t<Tscalar, Tcomplex>)));
    m_ofdmDeModdynDescprGpu =
        reinterpret_cast<ofdmDeModDynDescr_t<Tscalar, Tcomplex>*>(ofdmDeModdynDescprGpu);
    CHECK_CURESULT(cuMemcpyHtoD(
        reinterpret_cast<CUdeviceptr>(m_ofdmDeModdynDescprGpu),
        m_ofdmDeModdynDescprCpu,
        sizeof(ofdmDeModDynDescr_t<Tscalar, Tcomplex>)));
    ...
    if (m_prach) {// for PRACH
        grid_dim = dim3(m_ofdmDeModdynDescprCpu -> N_ueLayer,
                        (cuphyCarrierPrms->N_rep + OFDM_FFTs_PER_BLOCK_CONST_ - 1) /
                            OFDM_FFTs_PER_BLOCK_CONST_,
                        1);
    } else {// for non-PRACH
...
ofdmDeModulate<Tscalar, Tcomplex>::~ofdmDeModulate()
{
    ...
    if (m_cpInfoGpu) {
        CHECK_CURESULT_NOEXCEPT(cuMemFree(reinterpret_cast<CUdeviceptr>(m_cpInfoGpu)));
    }
    
    CHECK_CURESULT_NOEXCEPT(cuMemFree(reinterpret_cast<CUdeviceptr>(m_ofdmDeModdynDescprGpu)));
...
void ofdmDeModulate<Tscalar, Tcomplex>::determinePrachParam(cuphyCarrierPrms_t* cuphyCarrierPrms)
{
    ...
    double t_start_RA       = 0.0;
    double t_start_l_mu     = 0.0;
    uint32_t mu             = cuphyCarrierPrms->mu;
    ...
    uint32_t f_samp       = cuphyCarrierPrms->f_samp;
    ...
    for (int ll = 0; ll<=startRaSym; ll++) {
        ...
        if (ll == 0) {
            ...
            t_start_l_mu   += static_cast<double>(cuphyCarrierPrms->N_u_mu + N_CP_l_mu) * T_c;
        }
    }
    ...
    if (delta_f_RA == 1250 || delta_f_RA == 5000) {
        ...
        n = 0;
        double tempf = t_start_RA + static_cast<double>(N_u + N_CP_RA) * T_c;
    ...
    uint32_t preambleSampStart                  = static_cast<uint32_t>(llround(t_start_RA * static_cast<double>(f_samp))); // starting point right before CP
...
void ofdmDeModulate<Tscalar, Tcomplex>::printFreqSample(int printLen)
{
    ...
    CHECK_CURESULT(cuMemcpyDtoH(
        temp_CPU_buffer,
        reinterpret_cast<CUdeviceptr>(m_ofdmDeModdynDescprCpu -> freqDataOut),
        printLen * sizeof(Tcomplex)));
...
void ofdmDeModulate<Tscalar, Tcomplex>::saveOfdmDemodToH5File(cudaStream_t strm)
{
    ...
    CHECK_CURESULT(cuStreamSynchronize(strm)); // sync stream to ensure processing done
