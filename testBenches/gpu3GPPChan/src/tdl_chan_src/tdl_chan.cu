...
{
    if (tdlCfg->delayProfile < 'A' || tdlCfg->delayProfile > 'C') {
        throw std::invalid_argument("Only TDL delay profiles A, B, and C are supported");
    }
...
template <typename Tscalar, typename Tcomplex> 
void tdlChan<Tscalar, Tcomplex>::saveTdlChanToH5File(const std::string & padFileNameEnding)
{
    cudaStreamSynchronize(this -> m_strm);
