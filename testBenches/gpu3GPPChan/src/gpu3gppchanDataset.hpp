...
#ifndef GPU3GPPCHAN_DATASET_HPP
#define GPU3GPPCHAN_DATASET_HPP
...
struct UtParam : public UtParamCfg {
    ...
    float d_2d_in_o2i = -1.0f;  //!< Legacy Table 7.4.3-3 indoor distance (m; -1 means invalid)
...
struct SystemLevelConfig {
    ...
    static constexpr float MIN_ST_DIST_FROM_TX_RX_METERS{10.0F};
    ...
    uint8_t isac_type{0};  //!< ISAC type: 0: communication only (no sensing targets)
                           /*!< 1: single-cell TRP monostatic sensing (CPU-only);
                                2: reserved; bistatic sensing is currently rejected. */
    ...
    uint8_t st_size_ind{0};  //!< UAV size option: 0 = large, 1 = small
    float st_min_dist_from_tx_rx{MIN_ST_DIST_FROM_TX_RX_METERS};  //!< Minimum ST-to-STX/SRX 3D distance (m)
                                                                  /*!< Default: 10 meters for UAV per TR 38.901. */
    ...
    uint8_t st_rcs_model{1};  //!< Non-UAV RCS model selector. UAV model is derived
                              /*!< from st_size_ind per Tables 7.9.2.1-1/2. */
    ...
    uint8_t o2i_building_penetr_loss_ind = 1;  // 0: none, 1: low-loss, 2: 50% low-loss/50% high-loss, 3: high-loss (Table 7.4.3-2); 4: legacy Table 7.4.3-3, < 6 GHz only
...
struct LinkLevelConfig {
    ...
    char delay_profile = 'A';  // TDL/CDL: 'A' to 'C' (CDL-D/E LOS is not implemented)
...
struct SimConfig {
    ...
    // 0: external CIR/CFR, 1: internal CIR/external CFR, 2: internal CIR/CFR.
    // External arrays are host memory in CPU-only mode and device memory in GPU mode.
    int internal_memory_mode = 0;
