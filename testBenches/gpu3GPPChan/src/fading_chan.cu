...
m_enableSwapTxRx(0)
{
    ...
    CHECK_CURESULT(cuMemAllocHost(
        reinterpret_cast<void**>(&m_fadingChanDynDescrCpu),
        sizeof(fadingChanDynDescr_t<Tcomplex>)));
    CUdeviceptr fadingChanDynDescrGpu{};
    CHECK_CURESULT(cuMemAlloc(
        &fadingChanDynDescrGpu, sizeof(fadingChanDynDescr_t<Tcomplex>)));
    m_fadingChanDynDescrGpu = reinterpret_cast<fadingChanDynDescr_t<Tcomplex>*>(
        fadingChanDynDescrGpu);
}
...
fadingChan<Tcomplex>::~fadingChan()
{
    ...
    if (m_freqRxNoiseFree) {
        CHECK_CURESULT_NOEXCEPT(cuMemFree(reinterpret_cast<CUdeviceptr>(m_freqRxNoiseFree)));
    }

    CHECK_CURESULT_NOEXCEPT(cuMemFreeHost(m_fadingChanDynDescrCpu));
    CHECK_CURESULT_NOEXCEPT(cuMemFree(reinterpret_cast<CUdeviceptr>(m_fadingChanDynDescrGpu)));
}
...
void fadingChan<Tcomplex>::setup(hdf5hpp::hdf5_file& inputFile, uint8_t enableSwapTxRx)
{
    ...
    if (m_prach) { // PRACH
        ...
        CUdeviceptr freqRxNoiseFree{};
        CHECK_CURESULT(cuMemAlloc(
            &freqRxNoiseFree, sizeof(Tcomplex) * m_freqRxDataSizeUl));
        m_freqRxNoiseFree = reinterpret_cast<Tcomplex*>(freqRxNoiseFree);
        ...
        CHECK_CURESULT(cuMemAlloc(
            &freqRxNoiseFree,
            sizeof(Tcomplex) * std::max(m_freqRxDataSizeDl, m_freqRxDataSizeUl)));
        m_freqRxNoiseFree = reinterpret_cast<Tcomplex*>(freqRxNoiseFree);
        ...
        if(m_fadingMode == 1)
        {
            ...
            uint32_t timeTxLen = m_ofdmMod -> getTimeDataLen();
        ...
        else if (m_fadingMode == 2)
        {
            ...
            // get total time sample length
            uint32_t timeTxLen = m_ofdmMod -> getTimeDataLen();    // default is downlink
    ...
    CHECK_CURESULT(cuMemcpyHtoD(
        reinterpret_cast<CUdeviceptr>(m_fadingChanDynDescrGpu),
        m_fadingChanDynDescrCpu,
        sizeof(fadingChanDynDescr_t<Tcomplex>)));
}
    ...
    switch(m_fadingMode)
    {
        case 0: // AWGN, read freq rx from TV, float32 in TV
        {
            ...
            CHECK_CURESULT(cuMemcpyHtoDAsync(
                reinterpret_cast<CUdeviceptr>(m_freqRxNoiseFree),
                freqRxCpu,
                sizeof(Tcomplex) * freqRxDataSize,
                m_strm));
        ...
        case 2:  // CDL, read freq tx from TV, double in TV
        {
            ...
            CHECK_CURESULT(cuMemcpyHtoDAsync(
                reinterpret_cast<CUdeviceptr>(m_Tx),
                freqTxCpu,
                sizeof(Tcomplex) * freqTxDataSize,
                m_strm));
            delete[] readOutBuffer; 
            delete[] freqTxCpu;
            break;
...
void fadingChan<Tcomplex>::read_Xtf_prach(hdf5hpp::hdf5_file& inputFile)
{
    if (m_fadingMode == 1 || m_fadingMode == 2) { // TDL or CDL
        ...
        CHECK_CURESULT(cuMemcpyHtoDAsync(
            reinterpret_cast<CUdeviceptr>(m_Tx),
            timeTxCpu,
            sizeof(Tcomplex) * m_timeTxDataSizeUl,
            m_strm));
        delete[] readOutBuffer; 
        delete[] timeTxCpu;
    } else { // AWGN
        ...
        for(int i=0; i<m_freqRxDataSizeUl; i++)
            ...
            CHECK_CURESULT(cuMemcpyHtoDAsync(
                reinterpret_cast<CUdeviceptr>(m_freqRxNoiseFree),
                freqRxCpu,
                sizeof(Tcomplex) * m_freqRxDataSizeUl,
                m_strm));
            delete[] readOutBuffer; 
            delete[] freqRxCpu;
    }
...
void fadingChan<Tcomplex>::addNoiseFreq(float targetSNR)
{
    ...
    CHECK_CURESULT(cuMemcpyDtoDAsync(
        reinterpret_cast<CUdeviceptr>(m_freqRxNoisy),
        reinterpret_cast<CUdeviceptr>(m_freqRxNoiseFree),
        freqDataSize * sizeof(Tcomplex),
        m_strm));
...
void fadingChan<Tcomplex>::savefadingChanToH5File()
{
    ...
    uint8_t rank = 3;
    hsize_t dims[3] = {N_sc, N_symbol_slot, 1};
...
{
    const uint16_t nRxAnt = m_enableSwapTxRx
        ? m_carrierPrms->N_bsLayer
        : m_carrierPrms->N_ueLayer;
    uint16_t N_sc = m_carrierPrms -> N_sc;
    uint16_t N_symbol_slot = m_carrierPrms -> N_symbol_slot;
    if (ofdmSymIdx >= N_symbol_slot || startSC >= endSC || endSC > N_sc)
    {
        fprintf(
            stderr,
            "Invalid calSnr range: symbol=%u, startSC=%u, endSC=%u, "
            "N_sc=%u, N_symbol_slot=%u\n",
            ofdmSymIdx,
            startSC,
            endSC,
            N_sc,
    ...
    for(uint16_t rxAntIdx = 0; rxAntIdx < nRxAnt; rxAntIdx++)
    {
        uint32_t scOffset = (rxAntIdx * N_symbol_slot + ofdmSymIdx) * N_sc;
        float sigalSum = 0.0f;
        float noiseSum = 0.0f;
        Tcomplex tempSamp, tempNoise;
        for(uint32_t scIdx = startSC; scIdx < endSC; scIdx++)
        {
        ...
        for(uint32_t scIdx = 0; scIdx < N_sc; scIdx++)
        {
...
inline void tdl_pars_from_dataset_elem(tdlConfig_t * tdlCfg, const hdf5hpp::hdf5_dataset_elem& dset_elem)
{
    ...
    if (tdlCfg->fBatch == 0) {
        throw std::invalid_argument("TDL fBatch must be greater than zero");
    }
...
inline void cdl_pars_from_dataset_elem(cdlConfig_t * cdlCfg, const hdf5hpp::hdf5_dataset_elem& dset_elem)
{
    ...
    if (cdlCfg->fBatch == 0) {
        throw std::invalid_argument("CDL fBatch must be greater than zero");
    }
