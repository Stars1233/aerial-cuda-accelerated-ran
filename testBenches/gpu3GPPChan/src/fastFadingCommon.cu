...
void fastFadingBaseChan<Tscalar, Tcomplex>::setup()
{
    ...
    {
        if (m_fBatch <= 0.0f) {
            throw std::invalid_argument("fBatch must be greater than zero when batchLen is empty");
        }
...
{
#if USE_FFT_CAL_CFR
    if (m_fastFadingDynDescrCpu -> freqConvertType == 4)
    {
    ...
    else if(m_runMode == 2) // generate frequence channel on Sc and Prbg
    {
        ...
        blockDim = dim3(m_fastFadingDynDescrCpu -> N_Prbg, static_cast<uint32_t>(m_nBsAnt / m_scaleBsAntFreqChan), static_cast<uint32_t>(m_nUeAnt / m_scaleUeAntFreqChan));
    ...
    dim3 blockDim = dim3(m_fastFadingDynDescrCpu -> N_Prbg, static_cast<uint32_t>(m_nBsAnt / m_scaleBsAntFreqChan), static_cast<uint32_t>(m_nUeAnt / m_scaleUeAntFreqChan));
...
void fastFadingBaseChan<Tscalar, Tcomplex>::saveChanToH5File(hid_t & h5FileHandle, hid_t & complexDataType)
{
    ...
    if(m_sigLenPerAnt)
    {
        ...
        if (m_fastFadingDynDescrCpu -> saveAntPairSample == 1) // save per antenna sample
        {
            ...
            writeHdf5DatasetFromGpu<Tcomplex>(h5FileHandle, "rxSigOutPerAntPair", complexDataType, m_fastFadingDynDescrCpu -> rxSigOutPerAntPair, dims, rank);
        }
    }
}
...
fftKernelHandle<Tscalar, Tcomplex> fast_fading_get_fft_param(dim3& block_dim, uint32_t& shared_memory_size, uint8_t& runMode)
{
    using namespace cufftdx;
...
template<typename Tscalar, typename Tcomplex>
fftKernelHandle<Tscalar, Tcomplex> fast_fading_get_fft_param(const int Nfft, uint32_t cudaDeviceArch, dim3& block_dim, uint32_t& shared_memory_size, uint8_t& runMode)
{
