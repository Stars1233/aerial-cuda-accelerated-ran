...
// O2I penetration loss calculation defined in sls_chan_large_scale.cu.
float calPenetrLos(scenario_t scenario, bool outdoor_ind, float fc, float d_2d_in,
                   float d_2d_in_o2i,
                   std::uint8_t o2iBuildingPenetrLossInd,
                   std::uint8_t o2iCarPenetrLossInd, std::mt19937& gen,
    ...
    , m_totalSpstCount(0)
{
    ...
    if (m_antPanelConfig == nullptr || m_antPanelConfig->empty()) {
        throw std::invalid_argument("At least one antenna panel configuration is required");
    }
    const auto validate_panel_indices = [this](const auto& entities,
                                               const std::string_view entity_name) {
        for (const auto& entity : entities) {
            if (entity.antPanelIdx >= m_antPanelConfig->size()) {
                throw std::invalid_argument(
                    std::string(entity_name) + " antenna panel index is out of range");
            }
        }
    };
    validate_panel_indices(m_topology.cellParams, "Cell");
    validate_panel_indices(m_topology.utParams, "UT");

    // This implementation is calibrated only for the UAV target parameters
    // in TR 38.901 Section 7.9. Reject other enum values instead of silently
    // applying UAV XPR/RCS constants to unsupported target classes.
    if (m_sysConfig->isac_type > 0 &&
        std::any_of(
            m_topology.stParams.begin(), m_topology.stParams.end(),
            [](const StParam& target) {
                return target.target_type != SensingTargetType::UAV;
            })) {
        throw std::invalid_argument(
            "The ISAC channel model currently supports UAV sensing targets only");
    }
    if (m_sysConfig->isac_type > 0) {
        if (m_sysConfig->isac_type != 1) {
            throw std::invalid_argument(
                "The ISAC channel model currently supports TRP monostatic sensing only");
        }
        if (m_topology.cellParams.size() != 1) {
            throw std::invalid_argument(
                "The ISAC channel model currently supports one sensing cell only");
        }
        if (m_simConfig->cpu_only_mode != 1) {
            throw std::invalid_argument(
                "The ISAC channel model currently requires cpu_only_mode=1");
        }
    }
    ...
    if (m_simConfig->cpu_only_mode == 1) {
        ...
        initializeLargeScaleGpuKernels();
        initializeSmallScaleGpuKernels();
        generateCRNGPU();
        // Generate RP clusters after CRN is ready (monostatic ISAC only)
        if (m_sysConfig->isac_type == 1) {
            generateRpClusters();
...
{
    if (!activeCell.empty() && !activeUt.empty() && activeCell.size() != activeUt.size()) {
        throw std::invalid_argument("activeCell and activeUt must contain the same number of cell entries");
    }
    if (activeCell.empty() && !activeUt.empty() && activeUt.size() != m_topology.nSector) {
        throw std::invalid_argument(
            "activeUt must contain one entry per sector when activeCell is omitted");
    }
    for (const uint16_t cid : activeCell) {
        if (cid >= m_topology.nSector) {
            throw std::invalid_argument(
                "activeCell contains out-of-range cell ID " + std::to_string(cid));
        }
    }
    for (const auto& selectedUts : activeUt) {
        for (const uint16_t uid : selectedUts) {
            if (uid >= m_topology.nUT) {
                throw std::invalid_argument(
                    "activeUt contains out-of-range UT ID " + std::to_string(uid));
            }
        }
    }
    ...
    if (!utNewLoc.empty()) {
        ...
        m_updatePerTTILinkParams = true;
        // An explicit utNewLoc teleport (e.g. REM grid points) is an independent
        // location, so the LOS/NLOS condition must be recomputed here. Without this
        // the indicator stays frozen at the initial UT placement and every teleported
        // point inherits that first condition (observed: 100% LOS across a REM map).
        // Scoped to the teleport path only: the continuous-mobility branch below must
        // keep its spatially-consistent LOS state rather than redraw it each TTI.
        m_updateLosState = true;
    }
    ...
    if (activeUt.empty()) {
        const uint32_t nSelectedCells = activeCell.empty()
                                            ? m_topology.nSector
                                            : static_cast<uint32_t>(activeCell.size());
        nActiveLinks = nSelectedCells * m_topology.nUT;
    }
    else {
    ...
    // Update communication-link parameters if needed.
    ...
    if (m_sysConfig->isac_type > 0 && m_topology.nST > 0) {
        ...
        const bool targetPositionChanged =
            m_refTime > mobilityRefTime &&
            std::any_of(
                m_topology.stParams.begin(), m_topology.stParams.end(),
                [](const StParam& target) {
                    return target.velocity[0] != 0.0f || target.velocity[1] != 0.0f ||
        ...
        // TR 38.901 Section 7.9.5.1 requires the Option-2 random NLOS-ray
        // coupling to remain unchanged within a simulation drop, even when
        // positions change. Do not key sensing-link regeneration from the
        // communication-link flag: built-in UE mobility and active-link
        // changes set that flag every TTI and would redraw the target clusters,
        // coupling, RCS, XPR, and phases for a stationary target. Moving
        // targets rebuild their geometry while retaining the compatible
        // Option-2 permutation; reset() starts a new drop.
        const bool needTargetLinkUpdate =
            !continuous_fading || m_updateTargetLinkParams ||
            m_targetChannelParams.targetLinks.empty() || targetPositionChanged;
        if (needTargetLinkUpdate) {
            const bool preserveNlosCoupling =
                continuous_fading && targetPositionChanged &&
                !m_updateTargetLinkParams &&
                !m_targetChannelParams.targetLinks.empty();
            calculateAllTargetLinkParams(preserveNlosCoupling);
            m_targetChannelParams.nTargetLinks = static_cast<uint32_t>(m_targetChannelParams.targetLinks.size());
            m_updateTargetLinkParams = false;
        }
    ...
    if (m_simConfig->internal_memory_mode >= 1) {
        ...
        const auto hasBuffer = [](const auto& buffers) {
            return std::any_of(
                buffers.begin(), buffers.end(),
                [](const auto* buffer) { return buffer != nullptr; });
        };
        if (hasBuffer(cirCoePerCell) || hasBuffer(cirNormDelayPerCell) ||
            hasBuffer(cirNTapsPerCell) || hasBuffer(cfrScPerCell) ||
            hasBuffer(cfrPrbgPerCell)) {
            ...
            copyContiguousToPerCell(m_activeCell, m_activeUt);
        }
    }
...
void slsChan<Tscalar, Tcomplex>::saveSlsChanToH5File(std::string_view filenameEnding)
{
    ...
    if (slsH5File < 0) {
        throw std::runtime_error("Failed to create HDF5 file: " + outFilename);
    }

    // Files without this attribute use the legacy implicit v1 schema. Version
    // 2 identifies active-UT-sized per-cell datasets and corrected native
    // field widths. Version 3 makes ISAC delay dataset units explicit.
    constexpr std::uint32_t hdf5FormatVersion = 3;
    hid_t versionDataspace = H5Screate(H5S_SCALAR);
    hid_t versionAttribute = H5Acreate2(
        slsH5File, "gpu3gppchan_format_version", H5T_NATIVE_UINT32,
        versionDataspace, H5P_DEFAULT, H5P_DEFAULT);
    H5Awrite(versionAttribute, H5T_NATIVE_UINT32, &hdf5FormatVersion);
    H5Aclose(versionAttribute);
    H5Sclose(versionDataspace);
    ...
    if (h5_dump_level >= 1 && !m_clusterParams.empty() && m_clusterParams.size() >= nTotalLinks) {
        ...
        hsize_t randomPhasesDim[1] = {ClusterParams::MAX_SECTORS * ClusterParams::MAX_CLUSTERS * ClusterParams::MAX_RAYS * 4};
    ...
    const auto selectedCellId = [this](size_t selectionIndex) -> uint32_t {
        return selectionIndex < m_activeCell.size()
                   ? static_cast<uint32_t>(m_activeCell[selectionIndex])
                   : static_cast<uint32_t>(selectionIndex);
    };
    const auto hasPopulatedBuffer = [](const auto& buffers) {
        return std::any_of(
            buffers.begin(), buffers.end(),
            [](const auto* buffer) { return buffer != nullptr; });
    };
...
#endif
    if (hasPopulatedBuffer(m_cirCoePerCell)) {
        ...
        for (uint32_t cellIdx = 0; cellIdx < nCells; ++cellIdx) {
            if (m_cirCoePerCell[cellIdx] != nullptr) {
                const uint32_t cellId = selectedCellId(cellIdx);
                const uint32_t nActiveUt = cellIdx < m_activeUt.size()
                ...
                // Create dataset name
                std::string datasetName = "cirCoe_cell" + std::to_string(cellId);
                ...
                if (cellIdx < m_activeUt.size() && !m_activeUt[cellIdx].empty()) {
                    const std::string mappingName = "ue_mapping_cell" + std::to_string(cellId);
        ...
        if (hasPopulatedBuffer(m_cirNormDelayPerCell)) {
            for (uint32_t cellIdx = 0; cellIdx < nCells; ++cellIdx) {
                if (m_cirNormDelayPerCell[cellIdx] != nullptr) {
                    ...
                    std::string datasetName = "cirNormDelay_cell" + std::to_string(cellId);
        ...
        if (hasPopulatedBuffer(m_cirNtapsPerCell)) {
            for (uint32_t cellIdx = 0; cellIdx < nCells; ++cellIdx) {
                if (m_cirNtapsPerCell[cellIdx] != nullptr) {
                    ...
                    std::string datasetName = "cirNtaps_cell" + std::to_string(cellId);
...
#endif
    if (hasPopulatedBuffer(m_freqChanPrbgPerCell)) {
        const uint32_t nCells = static_cast<uint32_t>(m_freqChanPrbgPerCell.size());
        const uint32_t nUtsPerCell = m_topology.nUT;
        const uint32_t nSnapshots = m_simConfig->n_snapshot_per_slot;
        const uint32_t nPrbgCoeff = getEffectiveNUeAnt() * getEffectiveNBsAnt() * m_simConfig->n_prbg;
        ...
        for (uint32_t cellIdx = 0; cellIdx < nCells; ++cellIdx) {
            if (m_freqChanPrbgPerCell[cellIdx] != nullptr) {
                ...
                std::string datasetName = "cfrPrbg_cell" + std::to_string(cellId);
                
                hsize_t cfrDims[3] = {nActiveUt, nSnapshots, nPrbgCoeff};
                ...
                if (m_simConfig->cpu_only_mode == 0) {
                    // GPU memory - copy to host (include all snapshots)
                    hostCfrData.resize(static_cast<size_t>(nActiveUt) * nSnapshots * nPrbgCoeff);
                    CHECK_CUDAERROR(cudaMemcpy(hostCfrData.data(), m_freqChanPrbgPerCell[cellIdx], 
                                               static_cast<size_t>(nActiveUt) * nSnapshots * nPrbgCoeff * sizeof(Tcomplex),
                                               cudaMemcpyDeviceToHost));
...
#endif
    if (hasPopulatedBuffer(m_freqChanScPerCell)) {
        ...
        switch (m_simConfig->run_mode) {
            case 2:
            case 3:
                nScCoeff = getEffectiveNUeAnt() * getEffectiveNBsAnt() * m_simConfig->n_prb * 12; // PRBs * 12 SCs
                break;
            case 4:
                nScCoeff = getEffectiveNUeAnt() * getEffectiveNBsAnt() * m_simConfig->fft_size; // FFT size
                break;
            default:
                nScCoeff = 0;
                break;
        ...
        if (nScCoeff > 0) {
            ...
            for (uint32_t cellIdx = 0; cellIdx < nCells; ++cellIdx) {
                if (m_freqChanScPerCell[cellIdx] != nullptr) {
                    ...
                    std::string datasetName = "cfrSc_cell" + std::to_string(cellId);
                    
                    hsize_t cfrDims[3] = {nActiveUt, nSnapshots, nScCoeff};
                    hid_t cfrDataspace = H5Screate_simple(3, cfrDims, nullptr);
                    ...
                    if (m_simConfig->cpu_only_mode == 0) {
                        // GPU memory - copy to host (include all snapshots)
                        hostCfrData.resize(static_cast<size_t>(nActiveUt) * nSnapshots * nScCoeff);
                        CHECK_CUDAERROR(cudaMemcpy(hostCfrData.data(), m_freqChanScPerCell[cellIdx], 
                                                   static_cast<size_t>(nActiveUt) * nSnapshots * nScCoeff * sizeof(Tcomplex),
                                                   cudaMemcpyDeviceToHost));
    ...
    
    {
        ...
        if (!m_topology.cellParams.empty()) {
            ...
            H5Tinsert(cellParamType, "cid", HOFFSET(CellParam, cid), H5T_NATIVE_UINT32);
            ...
            const hsize_t orientationDims[1] = {3};
            hid_t orientationType = H5Tarray_create2(H5T_NATIVE_FLOAT, 1, orientationDims);
            H5Tinsert(cellParamType, "antPanelOrientation",
                      HOFFSET(CellParam, antPanelOrientation), orientationType);
        ...
        for (const auto& ut : m_topology.utParams) {
            ...
            d_2d_ins_o2i.push_back(ut.d_2d_in_o2i);
        }
        ...
        hid_t d2dO2iDataset = H5Dcreate2(utGroup, "d_2d_in_o2i", H5T_NATIVE_FLOAT, utDataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        H5Dwrite(d2dO2iDataset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, d_2d_ins_o2i.data());
        H5Dclose(d2dO2iDataset);
        ...
        if (m_sysConfig && m_sysConfig->isac_type > 0 && !m_targetChannelParams.targetLinks.empty()) {
            ...
            std::vector<float> total_pathloss, total_delay_ns;
            ...
            for (const auto& link : m_targetChannelParams.targetLinks) {
                ...
                total_delay_ns.push_back(link.total_delay_ns);
            ...
            saveVec("total_delay_ns", total_delay_ns);
            ...
            // Persist the coupled paths actually used by the target CIR. The
            // calibration analyzer needs these paths rather than the unrelated
            // communication-link clusterParams dataset.
            std::vector<uint32_t> calibration_group_ids;
            std::vector<float> calibration_delays_ns;
            std::vector<float> calibration_powers;
            std::vector<float> calibration_aod;
            std::vector<float> calibration_zod;
            std::vector<float> calibration_aoa;
            std::vector<float> calibration_zoa;

            float maximumPathPower = 0.0f;
            for (const auto& link : m_targetChannelParams.targetLinks) {
                for (const auto& path : link.coupled_paths) {
                    maximumPathPower = std::max(maximumPathPower, path.path_power);
                }
            }
            const float pathDropThreshold = maximumPathPower * std::pow(
                10.0f, -m_sysConfig->path_drop_threshold_db / 10.0f);
            const float sampleRate =
                m_simConfig->sc_spacing_hz * m_simConfig->fft_size;
            auto targetCirAcceptedDelay = [&](float delayNanoseconds) {
                const float rounded = std::round(
                    isacDelaySamples(delayNanoseconds, sampleRate));
                if (!std::isfinite(rounded) || rounded < 0.0f ||
                    rounded > static_cast<float>(
                                  std::numeric_limits<uint16_t>::max()) ||
                    m_targetCIR.cirNtaps == nullptr ||
                    m_targetCIR.cirNormDelay == nullptr) {
                    return false;
                }
                const auto delaySamples = static_cast<uint16_t>(rounded);
                return std::find(
                    m_targetCIR.cirNormDelay,
                    m_targetCIR.cirNormDelay + m_targetCIR.cirNtaps[0],
                    delaySamples) !=
                    m_targetCIR.cirNormDelay + m_targetCIR.cirNtaps[0];
            };

            for (const auto& link : m_targetChannelParams.targetLinks) {
                const float pathlossPower = std::pow(
                    10.0f, -link.total_pathloss / 10.0f);
                for (const auto& path : link.coupled_paths) {
                    const bool isSpecular =
                        path.incident_specular && path.scattered_specular;
                    if (!isSpecular && path.path_power < pathDropThreshold) {
                        continue;
                    }

                    const float delayNanoseconds =
                        calculateTargetPathDelayNanoseconds(link, path);
                    if (!targetCirAcceptedDelay(delayNanoseconds)) {
                        continue;
                    }

                    const auto& incidentClusters = link.incident.clusters;
                    const auto& scatteredClusters = link.scattered.clusters;
                    calibration_group_ids.push_back(0u);
                    calibration_delays_ns.push_back(delayNanoseconds);
                    calibration_powers.push_back(path.path_power * pathlossPower);
                    calibration_aod.push_back(
                        incidentClusters.ray_phi_AOD[path.c_inc][path.r_inc]);
                    calibration_zod.push_back(
                        incidentClusters.ray_theta_ZOD[path.c_inc][path.r_inc]);
                    calibration_aoa.push_back(
                        scatteredClusters.ray_phi_AOA[path.c_sca][path.r_sca]);
                    calibration_zoa.push_back(
                        scatteredClusters.ray_theta_ZOA[path.c_sca][path.r_sca]);
                }
            }

            constexpr uint32_t calibrationPathsSchemaVersion = 1u;
            hid_t calibrationSchemaSpace = H5Screate(H5S_SCALAR);
            hid_t calibrationSchemaDataset = H5Dcreate2(
                isacGroup, "calibration_paths_schema_version",
                H5T_NATIVE_UINT32, calibrationSchemaSpace, H5P_DEFAULT,
                H5P_DEFAULT, H5P_DEFAULT);
            H5Dwrite(calibrationSchemaDataset, H5T_NATIVE_UINT32, H5S_ALL,
                     H5S_ALL, H5P_DEFAULT, &calibrationPathsSchemaVersion);
            H5Dclose(calibrationSchemaDataset);
            H5Sclose(calibrationSchemaSpace);

            if (!calibration_group_ids.empty()) {
                const hsize_t pathCount = calibration_group_ids.size();
                hid_t pathDataspace = H5Screate_simple(1, &pathCount, nullptr);
                hid_t pathGroup = H5Gcreate2(
                    isacGroup, "calibrationPaths", H5P_DEFAULT, H5P_DEFAULT,
                    H5P_DEFAULT);
                auto savePathFloat = [&](const char* name, const std::vector<float>& data) {
                    hid_t dataset = H5Dcreate2(
                        pathGroup, name, H5T_NATIVE_FLOAT, pathDataspace,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
                    H5Dwrite(dataset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL,
                             H5P_DEFAULT, data.data());
                    H5Dclose(dataset);
                };
                hid_t groupDataset = H5Dcreate2(
                    pathGroup, "group_id", H5T_NATIVE_UINT32, pathDataspace,
                    H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
                H5Dwrite(groupDataset, H5T_NATIVE_UINT32, H5S_ALL, H5S_ALL,
                         H5P_DEFAULT, calibration_group_ids.data());
                H5Dclose(groupDataset);
                savePathFloat("delay_ns", calibration_delays_ns);
                savePathFloat("power", calibration_powers);
                savePathFloat("aod_deg", calibration_aod);
                savePathFloat("zod_deg", calibration_zod);
                savePathFloat("aoa_deg", calibration_aoa);
                savePathFloat("zoa_deg", calibration_zoa);
                H5Gclose(pathGroup);
        ...
        if (m_sysConfig && m_sysConfig->isac_type == 1 && !m_targetChannelParams.monostaticRefPoints.empty()) {
            ...
            // Section 7.9.6.2 requires background spreads to be evaluated
            // separately for each reference point. Store the generated RP
            // rays with a stable group ID so the analyzer can do that.
            std::vector<uint32_t> calibration_group_ids;
            std::vector<float> calibration_delays_ns;
            std::vector<float> calibration_powers;
            std::vector<float> calibration_aod;
            ...
            float minimumArrivalZenith = -std::numeric_limits<float>::infinity();
            switch (static_cast<Scenario>(m_sysConfig->scenario)) {
                case Scenario::UMi: minimumArrivalZenith = 50.0f; break;
                case Scenario::UMa: minimumArrivalZenith = 80.0f; break;
                case Scenario::RMa: minimumArrivalZenith = 90.0f; break;
                default: break;
            }

            uint32_t calibrationGroupId = 0u;
            const float sampleRate =
                m_simConfig->sc_spacing_hz * m_simConfig->fft_size;
            auto backgroundCirAcceptedDelay = [&](float delayNanoseconds) {
                const float rounded = std::round(
                    isacDelaySamples(delayNanoseconds, sampleRate));
                if (!std::isfinite(rounded) || rounded < 0.0f ||
                    rounded > static_cast<float>(
                                  std::numeric_limits<uint16_t>::max()) ||
                    m_monostaticBackgroundCIR.cirNtaps == nullptr ||
                    m_monostaticBackgroundCIR.cirNormDelay == nullptr) {
                    return false;
                }
                const auto delaySamples = static_cast<uint16_t>(rounded);
                return std::find(
                    m_monostaticBackgroundCIR.cirNormDelay,
                    m_monostaticBackgroundCIR.cirNormDelay +
                        m_monostaticBackgroundCIR.cirNtaps[0],
                    delaySamples) !=
                    m_monostaticBackgroundCIR.cirNormDelay +
                        m_monostaticBackgroundCIR.cirNtaps[0];
            };
            for (const auto& ref : m_targetChannelParams.monostaticRefPoints) {
                for (uint32_t rpIndex = 0; rpIndex < ISAC_NUM_REFERENCE_POINTS;
                     ++rpIndex, ++calibrationGroupId) {
                    const auto& rp = ref.background_rps[rpIndex];
                    const auto& clusters = rp.clusters;
                    const uint16_t raysPerCluster = clusters.n_rays_per_cluster;
                    if (raysPerCluster == 0) {
                        continue;
                    }
                    const float pathlossPower = std::pow(
                        10.0f, -(rp.pathloss + rp.shadow_fading) / 10.0f);
                    for (uint16_t cluster = 0; cluster < clusters.n_clusters;
                         ++cluster) {
                        const float rayPower = pathlossPower *
                            clusters.cluster_powers[cluster] /
                            static_cast<float>(raysPerCluster);
                        for (uint16_t ray = 0; ray < raysPerCluster; ++ray) {
                            const float arrivalZenith =
                                clusters.ray_theta_ZOD[cluster][ray];
                            if (arrivalZenith < minimumArrivalZenith) {
                                continue;
                            }
                            const float delayNanoseconds =
                                calculateBackgroundPathDelayNanoseconds(
                                    rp, cluster);
                            if (!backgroundCirAcceptedDelay(delayNanoseconds)) {
                                continue;
                            }
                            calibration_group_ids.push_back(calibrationGroupId);
                            calibration_delays_ns.push_back(delayNanoseconds);
                            calibration_powers.push_back(rayPower);
                            calibration_aod.push_back(
                                clusters.ray_phi_AOD[cluster][ray]);
                            calibration_zod.push_back(arrivalZenith);
                            calibration_aoa.push_back(
                                clusters.ray_phi_AOA[cluster][ray]);
                            calibration_zoa.push_back(
                                clusters.ray_theta_ZOA[cluster][ray]);
                        }
                    }
                }
            }
            ...
            hid_t calibrationSchemaSpace = H5Screate(H5S_SCALAR);
            hid_t calibrationSchemaDataset = H5Dcreate2(
                bgGroup, "calibration_paths_schema_version",
                H5T_NATIVE_UINT32, calibrationSchemaSpace, H5P_DEFAULT,
                H5P_DEFAULT, H5P_DEFAULT);
            H5Dwrite(calibrationSchemaDataset, H5T_NATIVE_UINT32, H5S_ALL,
                     H5S_ALL, H5P_DEFAULT, &calibrationPathsSchemaVersion);
            ...
            if (!calibration_group_ids.empty()) {
                ...
                hid_t pathDataspace = H5Screate_simple(1, &pathCount, nullptr);
                hid_t pathGroup = H5Gcreate2(
                    bgGroup, "calibrationPaths", H5P_DEFAULT, H5P_DEFAULT,
                    H5P_DEFAULT);
                auto savePathFloat = [&](const char* name, const std::vector<float>& data) {
                    hid_t dataset = H5Dcreate2(
                        pathGroup, name, H5T_NATIVE_FLOAT, pathDataspace,
...
slsChan<Tscalar, Tcomplex>::~slsChan()
{
    ...
    if (!cpuOnly) {
        ...
        if (m_d_losOverride) {
            CHECK_CURESULT_NOEXCEPT(cuMemFree(
        ...
        if (m_d_crnLos) {
            if (m_crnGridsAllocated) {
                const size_t nGrids = static_cast<size_t>(m_crnAllocatedNSite) * m_nCrnLosLsp;
                std::vector<float*> losGrids(nGrids);
                CHECK_CURESULT_NOEXCEPT(cuMemcpyDtoH(
                    losGrids.data(), reinterpret_cast<CUdeviceptr>(m_d_crnLos),
                    nGrids * sizeof(float*)));
                for (float* grid : losGrids) {
                    if (grid) {
                        CHECK_CURESULT_NOEXCEPT(cuMemFree(reinterpret_cast<CUdeviceptr>(grid)));
                    }
                }
            }
            CHECK_CURESULT_NOEXCEPT(cuMemFree(reinterpret_cast<CUdeviceptr>(m_d_crnLos)));
        }
        if (m_d_crnNlos) {
            if (m_crnGridsAllocated) {
                const size_t nGrids = static_cast<size_t>(m_crnAllocatedNSite) * m_nCrnNlosLsp;
                std::vector<float*> nlosGrids(nGrids);
                CHECK_CURESULT_NOEXCEPT(cuMemcpyDtoH(
                    nlosGrids.data(), reinterpret_cast<CUdeviceptr>(m_d_crnNlos),
                    nGrids * sizeof(float*)));
                for (float* grid : nlosGrids) {
                    if (grid) {
                        CHECK_CURESULT_NOEXCEPT(cuMemFree(reinterpret_cast<CUdeviceptr>(grid)));
                    }
                }
            }
            CHECK_CURESULT_NOEXCEPT(cuMemFree(reinterpret_cast<CUdeviceptr>(m_d_crnNlos)));
        }
        if (m_d_crnO2i) {
            if (m_crnGridsAllocated) {
                const size_t nGrids = static_cast<size_t>(m_crnAllocatedNSite) * m_nCrnO2iLsp;
                std::vector<float*> o2iGrids(nGrids);
                CHECK_CURESULT_NOEXCEPT(cuMemcpyDtoH(
                    o2iGrids.data(), reinterpret_cast<CUdeviceptr>(m_d_crnO2i),
                    nGrids * sizeof(float*)));
                for (float* grid : o2iGrids) {
                    if (grid) {
                        CHECK_CURESULT_NOEXCEPT(cuMemFree(reinterpret_cast<CUdeviceptr>(grid)));
                    }
                }
            }
            CHECK_CURESULT_NOEXCEPT(cuMemFree(reinterpret_cast<CUdeviceptr>(m_d_crnO2i)));
        }
        if (m_d_corrDistLos) {
            CHECK_CURESULT_NOEXCEPT(cuMemFree(reinterpret_cast<CUdeviceptr>(m_d_corrDistLos)));
        }
        if (m_d_corrDistNlos) {
            CHECK_CURESULT_NOEXCEPT(cuMemFree(reinterpret_cast<CUdeviceptr>(m_d_corrDistNlos)));
        }
        if (m_d_corrDistO2i) {
            CHECK_CURESULT_NOEXCEPT(cuMemFree(reinterpret_cast<CUdeviceptr>(m_d_corrDistO2i)));
        }
...
void slsChan<Tscalar, Tcomplex>::allocateStaticGpuMem()
{
    ...
    const uint8_t includeDeltaTau = m_sysConfig->enable_propagation_delay != 0 ? 1 : 0;
    m_nCrnLosLsp = 7 + includeDeltaTau;
    m_nCrnNlosLsp = 6 + includeDeltaTau;
    m_nCrnO2iLsp = 6 + includeDeltaTau;
    ...
    CUdeviceptr crnLosAllocation{};
    CUdeviceptr crnNlosAllocation{};
    CUdeviceptr crnO2iAllocation{};
    CHECK_CURESULT(cuMemAlloc(&crnLosAllocation, nSite * m_nCrnLosLsp * sizeof(float*)));
    CHECK_CURESULT(cuMemAlloc(&crnNlosAllocation, nSite * m_nCrnNlosLsp * sizeof(float*)));
    CHECK_CURESULT(cuMemAlloc(&crnO2iAllocation, nSite * m_nCrnO2iLsp * sizeof(float*)));
    m_d_crnLos = reinterpret_cast<float**>(crnLosAllocation);
    m_d_crnNlos = reinterpret_cast<float**>(crnNlosAllocation);
    m_d_crnO2i = reinterpret_cast<float**>(crnO2iAllocation);
    
    // Initialize CRN seed
    m_crnSeed = m_randSeed;  // Fixed seed for reproducible results
    
    // Allocate GPU memory for correlation distances
    CUdeviceptr corrDistLosAllocation{};
    CUdeviceptr corrDistNlosAllocation{};
    CUdeviceptr corrDistO2iAllocation{};
    CHECK_CURESULT(cuMemAlloc(&corrDistLosAllocation, m_nCrnLosLsp * sizeof(float)));
    CHECK_CURESULT(cuMemAlloc(&corrDistNlosAllocation, m_nCrnNlosLsp * sizeof(float)));
    CHECK_CURESULT(cuMemAlloc(&corrDistO2iAllocation, m_nCrnO2iLsp * sizeof(float)));
    m_d_corrDistLos = reinterpret_cast<float*>(corrDistLosAllocation);
    m_d_corrDistNlos = reinterpret_cast<float*>(corrDistNlosAllocation);
    m_d_corrDistO2i = reinterpret_cast<float*>(corrDistO2iAllocation);
    ...
    switch (m_sysConfig->scenario) {
        ...
        default:
            throw std::invalid_argument(
                "Invalid scenario: " + std::to_string(static_cast<int>(m_sysConfig->scenario)));
    }
...
void slsChan<Tscalar, Tcomplex>::updateActiveLinkInd(
    ...
    // Expand omitted selectors so the cell and UT lists are always aligned.
    m_activeCell = activeCell;
    m_activeUt = activeUt;

    if (m_activeCell.empty()) {
#ifdef SLS_DEBUG_
        printf("DEBUG: activeCell is empty, selecting all %u sectors\n", m_topology.nSector);
#endif
        m_activeCell.reserve(m_topology.nSector);
        for (uint16_t cid = 0; cid < m_topology.nSector; ++cid) {
            m_activeCell.push_back(cid);
        }
    }

    if (m_activeUt.empty()) {
        std::vector<uint16_t> allUts(m_topology.nUT);
        std::iota(allUts.begin(), allUts.end(), uint16_t{0});
        m_activeUt.assign(m_activeCell.size(), allUts);
    }

    if (m_activeCell.size() != m_activeUt.size()) {
        throw std::invalid_argument("Expanded active cell and UT selectors are not aligned");
    }

#ifdef SLS_DEBUG_
    printf("DEBUG: Active cells: [");
    for (size_t i = 0; i < m_activeCell.size(); ++i) {
        printf("%u", m_activeCell[i]);
        if (i + 1 < m_activeCell.size()) printf(", ");
    }
    printf("]\n");
    printf("DEBUG: Active UTs: [");
    for (size_t i = 0; i < m_activeUt.size(); ++i) {
        printf("{");
        for (size_t j = 0; j < m_activeUt[i].size(); ++j) {
            printf("%u", m_activeUt[i][j]);
            if (j + 1 < m_activeUt[i].size()) printf(", ");
        }
        printf("}");
        if (i + 1 < m_activeUt.size()) printf(", ");
    }
    printf("]\n");
#endif
    ...
    for (size_t i = 0; i < m_activeCell.size(); ++i) {
        ...
        if (i < m_activeUt.size() && !m_activeUt[i].empty()) {
                ...
                if (m_simConfig->internal_memory_mode >= 2) {
                    if (m_freqChanPrbg) {
                        const size_t freqChanPrbgOffset = linkIdx * m_simConfig->n_snapshot_per_slot * 
                                                         effRxAnt * effTxAnt * m_simConfig->n_prbg;
            ...
            for (uint16_t uid : m_activeUt[i]) {
                ...
                const uint32_t effRxAnt = (m_effectiveUeAnt > 0)
                                              ? m_effectiveUeAnt
                                              : m_cmnLinkParams.nUeAnt;
                const uint32_t effTxAnt = (m_effectiveBsAnt > 0)
                                              ? m_effectiveBsAnt
                ...
                if (m_simConfig->internal_memory_mode >= 1) {
                    ...
                    if (m_simConfig->internal_memory_mode == 2) {
                        ...
                        if (m_freqChanPrbg) {
                            freqChanPrbgOffset = linkIdx * m_simConfig->n_snapshot_per_slot * effRxAnt * effTxAnt * m_simConfig->n_prbg;
                        ...
                        if (m_freqChanSc) {
                            ...
                            uint32_t scPerLink = (m_simConfig->run_mode == 4) ? m_simConfig->fft_size : (m_simConfig->n_prb * 12);
                            freqChanScOffset = linkIdx * m_simConfig->n_snapshot_per_slot * effRxAnt * effTxAnt * scPerLink;
                        ...
                        if (i < m_freqChanPrbgPerCell.size() && m_freqChanPrbgPerCell[i] != nullptr) {
                            size_t freqChanPrbgOffset = utIdxInCell * m_simConfig->n_snapshot_per_slot * effRxAnt * effTxAnt * m_simConfig->n_prbg;
                            freqChanPrbgPtr = m_freqChanPrbgPerCell[i] + freqChanPrbgOffset;
                        }
                        if (i < m_freqChanScPerCell.size() && m_freqChanScPerCell[i] != nullptr) {
                            uint32_t scPerLink = (m_simConfig->run_mode == 4) ? m_simConfig->fft_size : (m_simConfig->n_prb * 12);
                            size_t freqChanScOffset = utIdxInCell * m_simConfig->n_snapshot_per_slot * effRxAnt * effTxAnt * scPerLink;
                    ...
                    const size_t cirCoeOffset = utIdxInCell * m_simConfig->n_snapshot_per_slot * effRxAnt * effTxAnt * maxTaps;
        ...
        for (size_t i = 0; i < m_cirCoePerCell.size(); i++) {
            if (m_cirCoePerCell[i] != nullptr) {
                ...
                size_t offset = (m_cirCoePerCell[i] - m_cirCoe) / 
                              (m_simConfig->n_snapshot_per_slot * m_effectiveUeAnt * m_effectiveBsAnt * maxTaps);
...
template <typename Tscalar, typename Tcomplex>
void slsChan<Tscalar, Tcomplex>::reconcileExternalUtO2iState(
    UtParam& ut, const bool outdoorIndChanged)
{
    if (m_sysConfig->o2i_building_penetr_loss_ind > LEGACY_O2I_BUILDING_PENETR_LOSS_IND) {
        throw std::invalid_argument(
            "o2i_building_penetr_loss_ind must be in the range [0, 4]");
    }

    if (m_sysConfig->o2i_building_penetr_loss_ind == LEGACY_O2I_BUILDING_PENETR_LOSS_IND &&
        (m_simConfig->center_freq_hz >= LEGACY_O2I_MAX_FREQUENCY_HZ ||
         (m_sysConfig->scenario != scenario_t::UMa && m_sysConfig->scenario != scenario_t::UMi))) {
        throw std::invalid_argument(
            "o2i_building_penetr_loss_ind=4 (legacy Table 7.4.3-3) is valid only for UMa/UMi below 6 GHz");
    }

    if (ut.outdoor_ind > 1) {
        throw std::invalid_argument(
            "External UT outdoor_ind must be 0 (indoor) or 1 (outdoor)");
    }

    if (ut.outdoor_ind == 1) {
        ut.d_2d_in = 0.0f;
        ut.d_2d_in_o2i = -1.0f;
        ut.o2i_penetration_loss = 0.0f;
        return;
    }

    if (m_topology.cellParams.empty()) {
        throw std::runtime_error(
            "Cannot derive indoor UT state without a configured cell");
    }

    float closestCellDistance = std::numeric_limits<float>::max();
    for (const auto& cell : m_topology.cellParams) {
        closestCellDistance = std::min(
            closestCellDistance, calculateDistance2D(cell.loc, ut.loc));
    }

    const bool invalidIndoorDistance =
        !std::isfinite(ut.d_2d_in) || ut.d_2d_in < 0.0f ||
        ut.d_2d_in > closestCellDistance;
    const bool legacyO2iDistanceRequired =
        m_sysConfig->o2i_building_penetr_loss_ind == LEGACY_O2I_BUILDING_PENETR_LOSS_IND &&
        m_simConfig->center_freq_hz < LEGACY_O2I_MAX_FREQUENCY_HZ &&
        (m_sysConfig->scenario == scenario_t::UMa || m_sysConfig->scenario == scenario_t::UMi);
    const bool invalidLegacyO2iDistance =
        legacyO2iDistanceRequired &&
        (!std::isfinite(ut.d_2d_in_o2i) || ut.d_2d_in_o2i < 0.0f ||
         ut.d_2d_in_o2i > LEGACY_O2I_MAX_INDOOR_DISTANCE_M);
    const bool regenerateIndoorDistance = outdoorIndChanged || invalidIndoorDistance;
    const bool regenerateLegacyO2iDistance = outdoorIndChanged || invalidLegacyO2iDistance;

    if (regenerateIndoorDistance) {
        constexpr int maxIterations = 1000;
        bool distanceGenerated = false;
        for (int iteration = 0; iteration < maxIterations; ++iteration) {
#ifdef CALIBRATION_CFG_
            const float candidate =
                m_uniformDist(m_gen) * m_topology.maxBsUeDist2dIndoor;
#else
            const float candidate =
                std::min(m_uniformDist(m_gen), m_uniformDist(m_gen)) *
                m_topology.maxBsUeDist2dIndoor;
#endif
            if (candidate <= closestCellDistance) {
                ut.d_2d_in = candidate;
                distanceGenerated = true;
                break;
            }
        }
        if (!distanceGenerated) {
            const float maximumIndoorDistance = std::min(
                closestCellDistance, m_topology.maxBsUeDist2dIndoor);
            ut.d_2d_in = m_uniformDist(m_gen) * maximumIndoorDistance;
        }
    }

    if (regenerateLegacyO2iDistance) {
        if (legacyO2iDistanceRequired) {
            ut.d_2d_in_o2i = LEGACY_O2I_MAX_INDOOR_DISTANCE_M * m_uniformDist(m_gen);
        } else {
            ut.d_2d_in_o2i = -1.0f;
        }
    }

    if (regenerateIndoorDistance || regenerateLegacyO2iDistance) {
        ...
        for (const auto& ext_ut : extConfig->ut_config) {
            ...
            for (auto& ut : m_topology.utParams) {
                if (ut.uid == ext_ut.uid) {
                    ...
                    reconcileExternalUtO2iState(ut, outdoorIndChanged);
                    break;
                }
            }
        }
...
void slsChan<Tscalar, Tcomplex>::dump_pl_sf_ant_gain_stats(float* pl_sf_ant_gain,
    ...
    for (uint32_t cell_idx = 0; cell_idx < num_cells; ++cell_idx) {
        ...
        for (uint32_t ue_idx = 0; ue_idx < num_uts; ++ue_idx) {
            ...
            // BS-side antenna gain: eq 7.1-7/-8 GCS->LCS mapping (same transform as the
            // coefficient path; psi is irrelevant for the scalar pattern gain)
            float bs_theta_deg, bs_phi_deg, bs_psi_unused;
            gcsToLcs(sector_orient_deg, m_topology.cellParams[cid].antPanelOrientation[0],
                     m_topology.cellParams[cid].antPanelOrientation[2],
                     m_linkParams[linkIdx].theta_LOS_ZOD, m_linkParams[linkIdx].phi_LOS_AOD,
                     bs_theta_deg, bs_phi_deg, bs_psi_unused);
            ...
            // Table 7.3-1: clamp the COMBINED attenuation to A_max before adding the gain
            float bs_ant_att = bs_panel->antTheta[bs_theta_idx] + bs_panel->antPhi[bs_phi_idx];
            if (bs_panel->antModel == 1) {
                bs_ant_att = std::max(bs_ant_att, -SLS_ANTENNA_PATTERN_A_MAX_DB);
            }
            float bs_ant_gain = bs_ant_att + (bs_panel->antModel == 1 ? G_max : 0.0f);
            ...
            // UE-side: same eq 7.1-7/-8 mapping with the UT panel orientation.
            // UT orientation[0] stores the boresight zenith angle (90 = horizontal),
            // so spec downtilt beta = orientation[0] - 90 (BS stores the downtilt directly).
            float ue_theta_deg, ue_phi_deg, ue_psi_unused;
            gcsToLcs(m_topology.utParams[uid].antPanelOrientation[1],
                     m_topology.utParams[uid].antPanelOrientation[0] - 90.0f,
                     m_topology.utParams[uid].antPanelOrientation[2],
                     m_linkParams[linkIdx].theta_LOS_ZOA, m_linkParams[linkIdx].phi_LOS_AOA,
                     ue_theta_deg, ue_phi_deg, ue_psi_unused);
            ...
            float ue_ant_att = ue_panel->antTheta[ue_theta_idx] + ue_panel->antPhi[ue_phi_idx];
            if (ue_panel->antModel == 1) {
                ue_ant_att = std::max(ue_ant_att, -SLS_ANTENNA_PATTERN_A_MAX_DB);
            }
            float ue_ant_gain = ue_ant_att + (ue_panel->antModel == 1 ? G_max : 0.0f);
...
template <typename Tscalar, typename Tcomplex>
slsChan<Tscalar, Tcomplex>*
createSlsChan(const SimConfig* simConfig,
              const SystemLevelConfig* sysConfig,
              const ExternalConfig* extConfig,
              uint32_t randSeed,
              cudaStream_t strm)
{
    return new slsChan<Tscalar, Tcomplex>(simConfig, sysConfig, extConfig, randSeed, strm);
}

template <typename Tscalar, typename Tcomplex>
void
destroySlsChan(slsChan<Tscalar, Tcomplex>* channel) noexcept
{
    delete channel;
}

template class slsChan<float, float2>;
template slsChan<float, float2>* createSlsChan<float, float2>(const SimConfig*,
                                                              const SystemLevelConfig*,
                                                              const ExternalConfig*,
                                                              uint32_t,
                                                              cudaStream_t);
template void destroySlsChan<float, float2>(slsChan<float, float2>*) noexcept;
