...
inline float sampleAnnulusRadius(float rmin, float rmax, float uniformSample)
{
    return std::sqrt(rmin * rmin + uniformSample * (rmax * rmax - rmin * rmin));
}
    ...
    int max_iterations = 1000)
{
    ...
    for (int iter = 0; iter < max_iterations; ++iter) {
        ...
        float randomDistance = sampleAnnulusRadius(rmin, rmax, uniformDist(gen));
    ...
    std::uniform_real_distribution<float>& uniformDist)
{
    ...
    // Sample uniformly by annulus area while respecting the exclusion radius.
    float r = sampleAnnulusRadius(min_dist_2d, radius, uniformDist(gen));
...
void slsChan<Tscalar, Tcomplex>::bsUeDropping()
{
    ...
    const bool useExternalCellTopology =
        m_extConfig != nullptr && !m_extConfig->cell_config.empty();
    
    if (m_topology.n_sector_per_site != 1 && m_topology.n_sector_per_site != 3) {
        throw std::runtime_error("n_sector_per_site must be either 1 or 3");
    }
    // The built-in multi-site topology is a tri-sector hexagonal grid. A
    // complete external cell topology may instead describe one sector per
    // site, as used by integrations that model every cell as its own site.
    if (m_topology.nSite > 1 && m_topology.n_sector_per_site != 3 &&
        !useExternalCellTopology) {
        throw std::runtime_error(
            "Multi-site single-sector deployments require an external cell topology");
    }
    ...
    switch (m_sysConfig->scenario) {
        ...
        case Scenario::UMi:
            ...
            break;
        case Scenario::RMa:
            if (m_sysConfig->isd != 1732.0f && m_sysConfig->isd != 5000.0f) {
                throw std::invalid_argument("RMa inter-site distance must be 1732 or 5000 meters");
            }
            ...
            break;
        default:
            throw std::invalid_argument("Unknown channel-model scenario");
    }
    ...
    if (useExternalCellTopology) {
        if (m_extConfig->cell_config.size() != m_topology.nSector) {
            throw std::runtime_error(
                "External cell_config size (" +
                std::to_string(m_extConfig->cell_config.size()) +
                ") must equal n_site * n_sector_per_site (" +
                std::to_string(m_topology.nSector) + ")");
        }

        // Store by cid because the rest of the channel model indexes
        // cellParams directly with a global cell ID.
        m_topology.cellParams.resize(m_topology.nSector);
        std::vector<bool> seenCell(m_topology.nSector, false);
        std::vector<uint32_t> sectorsPerSite(m_topology.nSite, 0);
        for (const CellParam& externalCell : m_extConfig->cell_config) {
            if (externalCell.cid >= m_topology.nSector) {
                throw std::runtime_error(
                    "External cell_config contains out-of-range cid " +
                    std::to_string(externalCell.cid));
            }
            if (seenCell[externalCell.cid]) {
                throw std::runtime_error(
                    "External cell_config contains duplicate cid " +
                    std::to_string(externalCell.cid));
            }
            if (externalCell.siteId >= m_topology.nSite) {
                throw std::runtime_error(
                    "External cell_config contains out-of-range siteId " +
                    std::to_string(externalCell.siteId));
            }
            const uint32_t expectedSiteId =
                externalCell.cid / m_topology.n_sector_per_site;
            if (externalCell.siteId != expectedSiteId) {
                throw std::runtime_error(
                    "External cell_config siteId must match cid / n_sector_per_site for cid " +
                    std::to_string(externalCell.cid));
            }
            seenCell[externalCell.cid] = true;
            ++sectorsPerSite[externalCell.siteId];
            m_topology.cellParams[externalCell.cid] = externalCell;
        }
        for (uint32_t siteIdx = 0; siteIdx < m_topology.nSite; ++siteIdx) {
            if (sectorsPerSite[siteIdx] != m_topology.n_sector_per_site) {
                throw std::runtime_error(
                    "External cell_config must provide " +
                    std::to_string(m_topology.n_sector_per_site) +
                    " sector(s) for site " + std::to_string(siteIdx));
            }
        }
    }
    else {
        // Generate the built-in BS positions in the 19-site hexagonal layout.
        for (uint32_t siteIdx = 0; siteIdx < m_sysConfig->n_site; siteIdx++) {
            ...
            if (siteIdx < 1) {
                ...
                throw std::runtime_error(
                    "The built-in topology supports at most 19 sites; provide external cell_config");
            }
            ...
            for (uint32_t sectorIdx = 0; sectorIdx < m_topology.n_sector_per_site; ++sectorIdx) {
                cell.cid = siteIdx * m_topology.n_sector_per_site + sectorIdx;
                cell.antPanelOrientation[1] = sectorOrien[sectorIdx];
    ...
    for (const uint32_t cid : dropCells) {
        const uint32_t siteIdx = m_topology.cellParams[cid].siteId;
    ...
    for (uint32_t uIdx = 0; uIdx < m_topology.nUT; uIdx++) {
        ...
        while(iterationCount < maxIterations) {
            ...
            if (useConfiguredUeDistRange) {
                randomDistance = sampleAnnulusRadius(cfgUeDistMin, cfgUeDistMax, m_uniformDist(m_gen));
            }
            else {
                ...
                if (ut.outdoor_ind == 0) {  // no min BS-UT distance for indoor UEs in calibration config
                    ...
                    randomDistance = sampleAnnulusRadius(
                        m_topology.minBsUeDist2d, cellRadius, m_uniformDist(m_gen));
                }
#else
        ...
        if (ut.outdoor_ind == 1) {
            ...
            const float closestCellDistance =
                calculateDistance2D(m_topology.cellParams[secIdx].loc, ut.loc);
...
static void configureSpstParams(StParam& st, uint8_t size_ind) {
    ...
    // For UAVs the size class selects the RCS model in TR 38.901: small UAVs
    // use Model 1 (Table 7.9.2.1-1) and large UAVs use Model 2
    // (Table 7.9.2.1-2). Keep the stored model aligned with the parameters
    // below so calculateRCS() cannot apply the Model-1 equation to a large UAV.
    st.rcs_model = effective_rcs_model;
...
void slsChan<Tscalar, Tcomplex>::stDropping()
{
    ...
    if (m_sysConfig->st_target_type == SensingTargetType::UAV &&
        m_sysConfig->st_size_ind > 1) {
        throw std::invalid_argument(
            "UAV st_size_ind must be 0 (large) or 1 (small)");
    }
    ...
    for (uint32_t st_idx = 0; st_idx < m_topology.nST; ++st_idx) {
        ...
        // The configured value is retained for non-UAV target types. For UAVs,
        // configureSpstParams() selects the spec-defined model from size_ind.
