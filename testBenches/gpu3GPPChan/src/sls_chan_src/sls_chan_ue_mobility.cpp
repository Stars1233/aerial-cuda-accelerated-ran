...
void move_stationary_ut(Coordinate& location, const float min_distance, std::mt19937& rng) {
    std::uniform_real_distribution<float> angle_dist(0.0F, 2.0F * static_cast<float>(M_PI));
    const float angle = angle_dist(rng);
    const float escape_distance = min_distance * 1.01F;
    location.x += std::cos(angle) * escape_distance;
    location.y += std::sin(angle) * escape_distance;
}
...
void updateUtLocation(UtParam& ut, const float current_time, const float ref_time,
    ...
    while (isUtTooCloseToSite(ut.loc, site_locations, min_distance) && 
        ...
        if (speed > 1e-6f) {
            ...
            const float escape_distance = min_distance * 0.1f;
            ...
            move_stationary_ut(ut.loc, min_distance, rng);
        }
