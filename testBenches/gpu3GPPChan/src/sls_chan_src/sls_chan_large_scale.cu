...
inline float calculateUMaLosPathloss(float d_2d, float d_3d, float h_bs, float h_ut, float fc,
    ...
    } else if (uniformDist(gen) <= prob_h_e) {
...
// NOTE: sigma_SF now follows Table 7.4.1-1 exactly. Earlier builds used 7 dB for every
// sub-6 GHz UMa/UMi link (LOS and NLOS), so SF statistics change when re-running
// existing sub-6 GHz configs.
...
float slsChan<Tscalar, Tcomplex>::calSfStd(scenario_t scenario, bool isLos, float fc, bool optionalPlInd, float d_2d, float h_bs, float h_ut,
    ...
    if (isIndoor && (scenario == scenario_t::UMa || scenario == scenario_t::UMi)) {
        return 7.0f;
    }
    if (isIndoor && scenario == scenario_t::RMa) {
        return 8.0f;
    }
    if (is_aerial) {
        h_ut = std::min(std::max((std::isnan(h_ut) || h_ut <= 0.0f) ? 10.001f : h_ut, 10.001f), 300.0f);
...
float calPenetrLos(scenario_t scenario, bool outdoor_ind, float fc, float d_2d_in,
    ...
    if (o2iBuildingPenetrLossInd > LEGACY_O2I_BUILDING_PENETR_LOSS_IND) {
        throw std::invalid_argument(
            "o2i_building_penetr_loss_ind must be in the range [0, 4]");
    }

    if (o2iBuildingPenetrLossInd == LEGACY_O2I_BUILDING_PENETR_LOSS_IND &&
        (fc >= LEGACY_O2I_MAX_FREQUENCY_HZ ||
         (scenario != scenario_t::UMa && scenario != scenario_t::UMi))) {
        throw std::invalid_argument(
    ...
    if (!outdoor_ind) {
        switch (scenario) {
            ...
            case scenario_t::UMi: {
                ...
                if (o2iBuildingPenetrLossInd == LEGACY_O2I_BUILDING_PENETR_LOSS_IND) {
                    if (!std::isfinite(d_2d_in_o2i) || d_2d_in_o2i < 0.0f ||
                        d_2d_in_o2i > LEGACY_O2I_MAX_INDOOR_DISTANCE_M) {
                        throw std::invalid_argument(
                            "d_2d_in_o2i must be in [0, 25] m for legacy O2I penetration loss");
                    }
                    ...
                    switch (o2iBuildingPenetrLossInd) {
                        ...
                        case 2:  // 50% low-loss, 50% high-loss building
                            if (uniformDist(gen) < 0.5f) {
                                ...
                                L_IRRglass = 23.0f + 0.3f * fc / 1e9f;   // Table 7.4.3-1 (Rel-18)
#else
                                L_IRRglass = 25.4f + 0.11f * fc / 1e9f;  // Table 7.4.3-1 (Rel-19, NOTE 2)
#endif
                                L_concreate = 5.0f + 4.0f * fc / 1e9f;    
...
// Also called from sls_chan_isac.cpp via a forward declaration -- needs external
// linkage, so must NOT be inline
float calPL(scenario_t scenario, bool isLos, float d_2d, float d_3d, float h_bs, float h_ut, float fc, 
    ...
    if (isLos) {
        switch (scenario) {
            ...
            case scenario_t::UMa:
                ...
                if (is_aerial && h_ut > 22.5f) {
                    ...
                    pl = calculateUMaLosPathloss(d_2d, d_3d, h_bs, h_ut, fc, gen, uniformDist, h_e_state);
                }
                break;
            case scenario_t::UMi:
        ...
        if (optionalPlInd && scenario != scenario_t::RMa) {
            ...
            switch (scenario) {
                ...
                case scenario_t::UMa: {
                    ...
                    if (is_aerial && h_ut > 22.5f) {
                        ...
                        float los_pl = calculateUMaLosPathloss(d_2d, d_3d, h_bs, h_ut, fc, gen, uniformDist, h_e_state);
...
inline float getScalingFactorAoaAod(const uint16_t nCluster) {
    for (int i = 0; i < N_SCALING_FACTORS_AOA_AOD; i++) {
        if (CLUSTER_COUNTS_AOA_AOD[i] == nCluster) {
            return SCALING_FACTORS_AOA_AOD[i];
        }
    }
...
inline float getScalingFactorZoaZod(const uint16_t nCluster) {
    for (int i = 0; i < N_SCALING_FACTORS_ZOA_ZOD; i++) {
        if (CLUSTER_COUNTS_ZOA_ZOD[i] == nCluster) {
            return SCALING_FACTORS_ZOA_ZOD[i];
        }
    }
    // If not found in table, throw an error
...
void slsChan<Tscalar, Tcomplex>::calCmnLinkParams()
{
    ...
    switch (m_sysConfig->scenario) {
        case scenario_t::UMa:
            ...
            // The DS/ASD/ASA/ZSA marginals and c_ASD below were updated by the Rel-19 study
            // on channel modelling enhancements for 7-24 GHz (Table 7.5-6 NOTE 8). Default is
            // TR 38.901 V19; define SLS_38901_REL18_PARAM to reproduce the Rel-18
            // values used by pre-Rel-19 calibration references.
...
#else
            m_cmnLinkParams.mu_lgDS[1] = -7.067f - 0.0794f * lgfc;  // LOS
            m_cmnLinkParams.mu_lgDS[0] = -6.47f - 0.134f * lgfc;    // NLOS
            m_cmnLinkParams.sigma_lgDS[1] = 0.57f + 0.026f * lgfc;
#endif
...
#else
            m_cmnLinkParams.mu_lgASD[2] = 0.58f;
            m_cmnLinkParams.mu_lgASD[1] = 0.92f;
            m_cmnLinkParams.mu_lgASD[0] = 1.09f;
            m_cmnLinkParams.sigma_lgASD[2] = 0.7f;
            m_cmnLinkParams.sigma_lgASD[1] = 0.31f;
            m_cmnLinkParams.sigma_lgASD[0] = 0.44f;
#endif
...
#else
            m_cmnLinkParams.mu_lgASA[1] = 1.76f;
            m_cmnLinkParams.mu_lgASA[0] = 2.04f - 0.25f * lgfc;
            m_cmnLinkParams.sigma_lgASA[1] = 0.19f;
            m_cmnLinkParams.sigma_lgASA[0] = 0.17f - 0.03f * lgfc;
#endif
...
#else
            m_cmnLinkParams.mu_lgZSA[1] = 0.96f;
            m_cmnLinkParams.mu_lgZSA[0] = 1.445f - 0.2856f * lgfc;
            m_cmnLinkParams.sigma_lgZSA[1] = 0.15f;
            m_cmnLinkParams.sigma_lgZSA[0] = 0.17f;
#endif
...
#else
            m_cmnLinkParams.C_ASD[2] = 1.8f;
            m_cmnLinkParams.C_ASD[1] = 3.58f;
            m_cmnLinkParams.C_ASD[0] = 1.8f;
#endif
        ...
        case scenario_t::UMi:
            ...
            // DS/ASD/ASA/ZSA marginals below were updated by the Rel-19 7-24 GHz study
            // (Table 7.5-6 NOTE 8). Default is TR 38.901 V19; define
            // SLS_38901_REL18_PARAM for the Rel-18 calibration values.
            m_cmnLinkParams.mu_lgDS[2] = -6.62f; // O2I
            m_cmnLinkParams.sigma_lgDS[2] = 0.32f;
#ifdef SLS_38901_REL18_PARAM
...
#else
            m_cmnLinkParams.mu_lgDS[1] = -7.28f - 0.18f * lg1fc;  // LOS
            m_cmnLinkParams.mu_lgDS[0] = -6.87f - 0.22f * lg1fc;  // NLOS
            m_cmnLinkParams.sigma_lgDS[1] = 0.39f;
            m_cmnLinkParams.sigma_lgDS[0] = 0.22f + 0.19f * lg1fc;
#endif

            m_cmnLinkParams.mu_lgASD[2] = 1.25f;
            m_cmnLinkParams.mu_lgASD[1] = 1.21f - 0.05f * lg1fc;  // unchanged in Rel-19 (NOTE 9)
            m_cmnLinkParams.sigma_lgASD[2] = 0.42f;
#ifdef SLS_38901_REL18_PARAM
...
#else
            m_cmnLinkParams.mu_lgASD[0] = 1.54f - 0.24f * lg1fc;
            m_cmnLinkParams.sigma_lgASD[1] = 0.29f + 0.08f * lg1fc;
            m_cmnLinkParams.sigma_lgASD[0] = 0.33f + 0.10f * lg1fc;
#endif
...
#else
            m_cmnLinkParams.mu_lgASA[1] = 1.66f - 0.07f * lg1fc;
            m_cmnLinkParams.mu_lgASA[0] = 1.76f - 0.07f * lg1fc;
            m_cmnLinkParams.sigma_lgASA[1] = 0.26f + 0.021f * lg1fc;
            m_cmnLinkParams.sigma_lgASA[0] = 0.27f + 0.05f * lg1fc;
#endif
...
#else
            m_cmnLinkParams.mu_lgZSA[1] = 0.81f - 0.11f * lg1fc;
            m_cmnLinkParams.mu_lgZSA[0] = 0.92f - 0.03f * lg1fc;
            m_cmnLinkParams.sigma_lgZSA[1] = 0.29f - 0.03f * lg1fc;
            m_cmnLinkParams.sigma_lgZSA[0] = 0.35f - 0.05f * lg1fc;
#endif
        ...
        case scenario_t::UMa:
            for (int i = 0; i < LOS_MATRIX_SIZE; i++) {
                for (int j = 0; j < LOS_MATRIX_SIZE; j++) {
                    m_cmnLinkParams.sqrtCorrMatLos[i * LOS_MATRIX_SIZE + j] = SQRT_CORR_MAT_UMA_LOS[i][j];
                }
            }
            for (int i = 0; i < NLOS_MATRIX_SIZE; i++) {
                for (int j = 0; j < NLOS_MATRIX_SIZE; j++) {
                    m_cmnLinkParams.sqrtCorrMatNlos[i * NLOS_MATRIX_SIZE + j] = SQRT_CORR_MAT_UMA_NLOS[i][j];
                }
            }
            for (int i = 0; i < O2I_MATRIX_SIZE; i++) {
                for (int j = 0; j < O2I_MATRIX_SIZE; j++) {
                    m_cmnLinkParams.sqrtCorrMatO2i[i * O2I_MATRIX_SIZE + j] = SQRT_CORR_MAT_UMA_O2I[i][j];
                }
            }
            break;
        case scenario_t::UMi:
            for (int i = 0; i < LOS_MATRIX_SIZE; i++) {
                for (int j = 0; j < LOS_MATRIX_SIZE; j++) {
                    m_cmnLinkParams.sqrtCorrMatLos[i * LOS_MATRIX_SIZE + j] = SQRT_CORR_MAT_UMI_LOS[i][j];
                }
            }
            for (int i = 0; i < NLOS_MATRIX_SIZE; i++) {
                for (int j = 0; j < NLOS_MATRIX_SIZE; j++) {
                    m_cmnLinkParams.sqrtCorrMatNlos[i * NLOS_MATRIX_SIZE + j] = SQRT_CORR_MAT_UMI_NLOS[i][j];
                }
            }
            for (int i = 0; i < O2I_MATRIX_SIZE; i++) {
                for (int j = 0; j < O2I_MATRIX_SIZE; j++) {
                    m_cmnLinkParams.sqrtCorrMatO2i[i * O2I_MATRIX_SIZE + j] = SQRT_CORR_MAT_UMI_O2I[i][j];
                }
            }
            break;
        case scenario_t::RMa:
            for (int i = 0; i < LOS_MATRIX_SIZE; i++) {
                for (int j = 0; j < LOS_MATRIX_SIZE; j++) {
                    m_cmnLinkParams.sqrtCorrMatLos[i * LOS_MATRIX_SIZE + j] = SQRT_CORR_MAT_RMA_LOS[i][j];
                }
            }
            for (int i = 0; i < NLOS_MATRIX_SIZE; i++) {
                for (int j = 0; j < NLOS_MATRIX_SIZE; j++) {
                    m_cmnLinkParams.sqrtCorrMatNlos[i * NLOS_MATRIX_SIZE + j] = SQRT_CORR_MAT_RMA_NLOS[i][j];
                }
            }
            for (int i = 0; i < O2I_MATRIX_SIZE; i++) {
                for (int j = 0; j < O2I_MATRIX_SIZE; j++) {
                    m_cmnLinkParams.sqrtCorrMatO2i[i * O2I_MATRIX_SIZE + j] = SQRT_CORR_MAT_RMA_O2I[i][j];
                }
            }
            break;
        default:
...
void slsChan<Tscalar, Tcomplex>::generateCRN()
{
    ...
    const CorrDist& corr_dist_los = (m_sysConfig->scenario == scenario_t::UMa) ? CORR_DIST_UMA_LOS :
                                (m_sysConfig->scenario == scenario_t::UMi) ? CORR_DIST_UMI_LOS : CORR_DIST_RMA_LOS;
    const CorrDist& corr_dist_nlos = (m_sysConfig->scenario == scenario_t::UMa) ? CORR_DIST_UMA_NLOS :
                                    (m_sysConfig->scenario == scenario_t::UMi) ? CORR_DIST_UMI_NLOS : CORR_DIST_RMA_NLOS;
    const CorrDist& corr_dist_o2i = (m_sysConfig->scenario == scenario_t::UMa) ? CORR_DIST_UMA_O2I :
                                    (m_sysConfig->scenario == scenario_t::UMi) ? CORR_DIST_UMI_O2I : CORR_DIST_RMA_O2I;
    ...
    for (uint16_t siteIdx = 0; siteIdx < nSite; siteIdx++) {
        ...
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_los.sf, m_gen, m_normalDist, m_crnLos[siteIdx][0]);
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_los.k, m_gen, m_normalDist, m_crnLos[siteIdx][1]);
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_los.ds, m_gen, m_normalDist, m_crnLos[siteIdx][2]);
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_los.asd, m_gen, m_normalDist, m_crnLos[siteIdx][3]);
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_los.asa, m_gen, m_normalDist, m_crnLos[siteIdx][4]);
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_los.zsd, m_gen, m_normalDist, m_crnLos[siteIdx][5]);
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_los.zsa, m_gen, m_normalDist, m_crnLos[siteIdx][6]);
        if (includeDeltaTau) {
            genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_los.dt, m_gen, m_normalDist, m_crnLos[siteIdx][7]);  // Delta Tau
        }

        // Generate CRN for NLOS case for this site
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_nlos.sf, m_gen, m_normalDist, m_crnNlos[siteIdx][0]);
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_nlos.ds, m_gen, m_normalDist, m_crnNlos[siteIdx][1]);
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_nlos.asd, m_gen, m_normalDist, m_crnNlos[siteIdx][2]);
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_nlos.asa, m_gen, m_normalDist, m_crnNlos[siteIdx][3]);
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_nlos.zsd, m_gen, m_normalDist, m_crnNlos[siteIdx][4]);
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_nlos.zsa, m_gen, m_normalDist, m_crnNlos[siteIdx][5]);
        if (includeDeltaTau) {
            genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_nlos.dt, m_gen, m_normalDist, m_crnNlos[siteIdx][6]);  // Delta Tau
        }

        // Generate CRN for O2I case for this site
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_o2i.sf, m_gen, m_normalDist, m_crnO2i[siteIdx][0]);
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_o2i.ds, m_gen, m_normalDist, m_crnO2i[siteIdx][1]);
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_o2i.asd, m_gen, m_normalDist, m_crnO2i[siteIdx][2]);
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_o2i.asa, m_gen, m_normalDist, m_crnO2i[siteIdx][3]);
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_o2i.zsd, m_gen, m_normalDist, m_crnO2i[siteIdx][4]);
        genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_o2i.zsa, m_gen, m_normalDist, m_crnO2i[siteIdx][5]);
        if (includeDeltaTau) {
            genCRN(m_maxX, m_minX, m_maxY, m_minY, corr_dist_o2i.dt, m_gen, m_normalDist, m_crnO2i[siteIdx][6]);  // Delta Tau
        }
    }
}
    ...
    for(uint16_t siteIdx = 0; siteIdx < m_topology.nSite; siteIdx++) {
        for(uint16_t ueIdx = 0; ueIdx < m_topology.nUT; ueIdx++) {
            ...
            if (m_updateLosState) {
                ...
                const uint8_t losDrawn = (m_uniformDist(m_gen) <= losProb) ? 1 : 0;
                const bool hasLosOverride =
                    linkIdx < m_losOverride.size() &&
                    m_losOverride[linkIdx] != SLS_LOS_OVERRIDE_NONE;
                m_linkParams[linkIdx].losInd = hasLosOverride
                    ? static_cast<uint8_t>(m_losOverride[linkIdx] != 0)
                    : losDrawn;
                m_linkParams[linkIdx].h_e = 0.0f;  // re-arm the once-per-drop h_E draw with the LOS state
            }
            ...
            if (m_updatePLAndPenetrationLoss) {
                float pl = calPL(m_sysConfig->scenario, m_linkParams[linkIdx].losInd, d_2d, d_3d, 
                            ...
                            m_linkParams[linkIdx].h_e, is_aerial);
            ...
            if (m_updatePLAndPenetrationLoss) {
                ...
                m_linkParams[linkIdx].SF = corrVars[SF_IDX] * calSfStd(m_sysConfig->scenario, losInd, m_simConfig->center_freq_hz, m_sysConfig->optional_pl_ind, d_2d, h_bs, h_ut, is_aerial, m_topology.utParams[ueIdx].outdoor_ind == 0);
            }
