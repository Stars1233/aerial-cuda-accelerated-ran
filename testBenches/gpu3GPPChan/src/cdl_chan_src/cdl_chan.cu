...
{
    if (cdlCfg == nullptr) {
        throw std::invalid_argument("cdlCfg must not be null");
    }
    if (cdlCfg->bsAntSize.size() != 5 || cdlCfg->ueAntSize.size() != 5 ||
        cdlCfg->bsAntSpacing.size() != 4 || cdlCfg->ueAntSpacing.size() != 4 ||
        cdlCfg->bsAntPolarAngles.size() != 2 || cdlCfg->ueAntPolarAngles.size() != 2 ||
        cdlCfg->vDirection.size() < 2) {
        throw std::invalid_argument(
            "CDL antenna sizes, spacings, polarization angles, and velocity direction "
            "must contain 5, 4, 2, and at least 2 entries, respectively");
    }
    if (cdlCfg->delayProfile < 'A' || cdlCfg->delayProfile > 'C') {
        throw std::invalid_argument(
            "Only CDL delay profiles A, B, and C are supported; "
            "CDL-D/E require the unimplemented specular LOS component");
    }
...
void cdlChan<Tscalar, Tcomplex>::genTimeChan()
{
    ...
    this -> m_blockDim = {m_numTaps, static_cast<uint32_t>(this -> m_nBsAnt / this -> m_scaleBsAntTimeChan), static_cast<uint32_t>(this -> m_nUeAnt / this -> m_scaleUeAntTimeChan)};
...
void cdlChan<Tscalar, Tcomplex>::reset()
{
    ...
    const uint32_t bs_ant_threads = this -> m_nBsAnt / this -> m_scaleBsAntTimeChan;
    const uint32_t ue_ant_threads = this -> m_nUeAnt / this -> m_scaleUeAntTimeChan;
    this -> m_blockDim = {m_numTaps, bs_ant_threads, ue_ant_threads};
    if (bs_ant_threads > 1)
    {
        this -> m_blockDim.y = bs_ant_threads / 2;
    }
    else if (ue_ant_threads > 1)
    {
        this -> m_blockDim.z = ue_ant_threads / 2;
    }
...
template <typename Tscalar, typename Tcomplex> 
void cdlChan<Tscalar, Tcomplex>::saveCdlChanToH5File(const std::string & padFileNameEnding)
{
    cudaStreamSynchronize(this -> m_strm);
...
// Explicit instantiations are defined once in this translation unit.
