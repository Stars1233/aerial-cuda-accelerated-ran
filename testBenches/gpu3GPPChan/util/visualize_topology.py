...
def visualize_topology(
    ...
    bs_label_added = False
    for loc in loc_to_cids:
        x, y = loc
        orientations_deg = sorted(set(loc_to_orientations.get(loc, [])))
        
        # Add the legend entry to the first BS even when no site is at the origin.
        ax.scatter(x, y, c=bs_color, marker='^', s=bs_marker_size,
                   label='BS' if not bs_label_added else '')
        bs_label_added = True
