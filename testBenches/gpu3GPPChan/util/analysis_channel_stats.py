...
- RSRP-based (default): Uses path loss, shadow fading, and BS antenna gain
...
# Legend label for the simulation curve; main() overrides via --backend (gpu|cpu)
SIM_LABEL = 'Simulation GPU'
...
class AntennaPanel:
    ...
    def antenna_gain_db(self, use_calibration_values: bool = False, calibration_phase: int = 2,
        ...
        if calibration_phase == 1:
            ...
            # Phase 2: CSI-RS port = one polarization's vertical column, K = M = 8 elements
            # (Table 7.8-2 TXRU virtualization; (M,N,P) = (8,4,2)). The previous 16-element
            # port merged both X-pol branches into one port, granting +10*log10(2) = 3.01 dB
            # of non-physical coherent cross-pol gain -- the systematic +3 dB offset vs the
            # 3GPP reference coupling-loss curve (G4 convention item).
            n_elements_per_port = 8
            if self.total_antennas < n_elements_per_port:
            ...
            max_gain_linear = 0.0
            best_angle = 0.0
            signal_angle_rad = np.deg2rad(angle_aod_deg - sector_orientation_deg)
            element_indices = np.arange(n_elements_per_port)
            steering_response = np.exp(
                1j * np.pi * np.cos(signal_angle_rad) * element_indices
            )

            # This geometry-only calculation has no per-port channel or
            # polarization input, so every idealized 8-element port has the
            # same array-factor magnitude. Actual port selection is performed
            # by _apply_virtualization_to_cir(), where each H_port is available.
            
            for sweep_angle_deg in angle_sweep_deg:
                absolute_angle_deg = sector_orientation_deg + sweep_angle_deg
                beam_angle_rad = np.deg2rad(sweep_angle_deg)
                w_port = (1.0 / np.sqrt(n_elements_per_port)) * np.exp(
                    1j * np.pi * np.cos(beam_angle_rad) * element_indices
                )
                beam_gain_linear = np.abs(np.vdot(w_port, steering_response)) ** 2

                if beam_gain_linear > max_gain_linear:
                    max_gain_linear = beam_gain_linear
            ...
            logger.debug(f"Panel {self.panel_idx}: Phase 2 virtualization, {n_tx_ports} TX ports, "
                         f"equivalent idealized port response, best_angle={best_angle:.1f}°, "
...
class H5ChannelAnalyzer:
    ...
    def __init__(self, h5_file_path: str, reference_json_path: str = None, calibration_phase: int = 2,
        ...
        self._phase2_best_beam_powers: Optional[Dict[int, Dict[int, float]]] = None
    ...
    def parse_h5_file(self) -> None:
        ...
        try:
            with h5py.File(self.h5_file_path, 'r') as f:
                format_version = int(f.attrs.get('gpu3gppchan_format_version', 1))
                logger.info(f"gpu3gppchan HDF5 format version: {format_version}")
                if format_version > 3:
                    logger.warning(
                        f"HDF5 format version {format_version} is newer than this reader supports"
                    )
                ...
                for cell_idx in range(self.system_level_config.n_site * self.system_level_config.n_sector_per_site):
                    ...
                    for ue_idx in range(cir_complex.shape[0]):
                        ...
                        if use_virtualization:
                            # Antenna virtualization: keep the BS-side array power, but report
                            # per UE receive branch (RSRP-style, average over RX branches = sum
                            # / n_rx). The 3GPP reference reports coupling loss per RX port;
                            # summing both X-pol UE branches sat exactly +10*log10(2) = 3.01 dB
                            # above the reference curve (G4 convention item).
                            n_rx_ant = ue_cir.shape[1] if ue_cir.ndim == 4 else (
                                self.system_level_config.n_ue_ant if hasattr(self.system_level_config, 'n_ue_ant') else 2)
                            n_tx_ant = self.system_level_config.n_bs_ant if hasattr(self.system_level_config, 'n_bs_ant') else 2
                            final_power = avg_power / n_rx_ant
                            logger.debug(f"Using antenna virtualization (per RX branch): n_rx={n_rx_ant}, n_tx={n_tx_ant}, final_power={final_power:.2e}")
                        else:
    ...
    def find_serving_cell(self, association_method: str = 'rsrp') -> int:
        ...
        Args:
            association_method: Association method to use:
                - 'rsrp': RSRP-based (pathloss, shadow fading, and BS gain) [default]
    ...
    def _find_serving_cell_by_rsrp(self) -> int:
        ...
        """
        serving_assignments = self._compute_rsrp_serving_assignments()
        ...
        serving_cells = list(serving_assignments.values())
        serving_cell_id = max(set(serving_cells), key=serving_cells.count)
        logger.info(f"Serving cell (RSRP+Azimuth): Cell {serving_cell_id}")
        return serving_cell_id

    def _compute_rsrp_serving_assignments(self) -> Dict[int, int]:
        """Return per-UE RSRP assignments with co-sited-sector tie-breaking."""
        ue_best_cells = {}
        best_beam_powers = {}
        uses_phase2_beams = self._uses_phase2_best_beam_power('rsrp')
        if uses_phase2_beams:
            best_beam_powers = self._get_phase2_best_beam_powers()
        cell_site_ids = {cell.cid: cell.site_id for cell in self.cell_params}
        ...
        for active_link in self.active_link_params:
            ...
            if uses_phase2_beams:
                # Full calibration attaches by large-scale RSRP.  Co-sited
                # sectors have the same pathloss and shadow fading, so their
                # actual best-of-12 CIR power is used below as the tie-breaker.
                rsrp = -link.pathloss + link.sf
            else:
                antenna_gain_db = self.get_antenna_gain_for_cell(
                    active_link.cid,
                    active_link.uid,
                    use_virtualization=self.calibration_phase in (1, 2),
                )
                rsrp = -link.pathloss + link.sf + antenna_gain_db
            az_diff = self._get_azimuth_alignment_for_link(active_link, link)
            previous = ue_best_cells.get(active_link.uid)

            if previous is None:
                ue_best_cells[active_link.uid] = (active_link.cid, rsrp, az_diff)
                continue

            _, previous_rsrp, previous_az_diff = previous
            previous_cell_id = previous[0]
            same_site = (
                active_link.cid in cell_site_ids
                and previous_cell_id in cell_site_ids
                and cell_site_ids[active_link.cid] == cell_site_ids[previous_cell_id]
            )
            if same_site and abs(rsrp - previous_rsrp) < 0.1:
                ue_beam_powers = best_beam_powers.get(active_link.uid, {})
                current_beam_power = ue_beam_powers.get(active_link.cid, 0.0)
                previous_beam_power = ue_beam_powers.get(previous_cell_id, 0.0)
                if current_beam_power > previous_beam_power:
                    ue_best_cells[active_link.uid] = (active_link.cid, rsrp, az_diff)
                elif current_beam_power == previous_beam_power and az_diff is not None and (
                        previous_az_diff is None or az_diff < previous_az_diff):
                    ue_best_cells[active_link.uid] = (active_link.cid, rsrp, az_diff)
            elif rsrp > previous_rsrp:
                ue_best_cells[active_link.uid] = (active_link.cid, rsrp, az_diff)

        return {ue_id: cell_info[0] for ue_id, cell_info in ue_best_cells.items()}

    def _uses_phase2_best_beam_power(self, association_method: str) -> bool:
        """Whether UMa Phase-2 analysis uses CIR-derived best-beam power."""
        return (
            self.calibration_phase == 2
            and association_method == 'rsrp'
            and self.system_level_config is not None
            and self.system_level_config.scenario == 'UMa'
            and not (self.isac_config and self.isac_config.is_enabled)
        )

    def _get_complete_phase2_best_beam_powers(
        self,
        best_beam_powers: Dict[int, Dict[int, float]],
        ue_id: int,
    ) -> Optional[Dict[int, float]]:
        """Return per-cell beam powers only when every usable UE link is covered."""
        expected_cell_ids = {
            active_link.cid
            for active_link in self.active_link_params
            if active_link.uid == ue_id
            and active_link.lsp_read_idx < len(self.link_params)
        }
        ue_powers = best_beam_powers.get(ue_id)
        if (
            ue_powers
            and expected_cell_ids
            and expected_cell_ids.issubset(ue_powers)
        ):
            return ue_powers
        return None
    ...
    def compute_wideband_sir(self, skip_isac_check: bool = False,
                             association_method: str = 'rsrp') -> float:
        """Compute the mean per-UE wideband Signal-to-Interference Ratio (SIR).

        Each UE's serving-cell signal is compared only with that UE's
        non-serving links. Noise is intentionally excluded.

        Args:
            skip_isac_check: Compatibility argument retained for existing callers;
                it currently has no effect.
            association_method: Serving-cell selection method: ``rsrp``, ``cir``,
                or ``distance``.

        Returns:
            Mean finite SIR in dB, positive infinity when every valid serving
            link is interference-free, or zero when no serving signal can be
            determined.

        Raises:
            KeyError: If a rich serving-cell assignment lacks its ``cell_id``.

        Examples:
            >>> analyzer.compute_wideband_sir(association_method='rsrp')  # doctest: +SKIP
            12.5
        """
        ...
        best_beam_powers = {}
        if self._uses_phase2_best_beam_power(association_method):
            best_beam_powers = self._get_phase2_best_beam_powers()
        sir_values_db = []
        interference_free_ues = 0

        for ue_id, assignment in serving_assignments.items():
            serving_cell_id = assignment['cell_id'] if isinstance(assignment, dict) else assignment

            ue_powers = self._get_complete_phase2_best_beam_powers(
                best_beam_powers, ue_id
            )
            if ue_powers is not None:
                signal_power_linear = ue_powers.get(serving_cell_id, 0.0)
                interference_power_linear = sum(
                    power for cell_id, power in ue_powers.items()
                    if cell_id != serving_cell_id
                )
                if signal_power_linear <= 0.0:
                    logger.warning(
                        "Could not determine signal power for UE %d", ue_id
                    )
                    continue
                if interference_power_linear <= 0.0:
                    interference_free_ues += 1
                    continue
                sir_values_db.append(
                    10 * np.log10(signal_power_linear / interference_power_linear)
                )
                continue
            ...
            for active_link in self.active_link_params:
                if active_link.uid != ue_id or active_link.lsp_read_idx >= len(self.link_params):
                    continue
                ...
                received_power_db = -link.pathloss + link.sf + antenna_gain_db
            ...
            if signal_power_db is None:
                logger.warning(f"Could not determine signal power for UE {ue_id}")
                continue
            ...
            sir_values_db.append(10 * np.log10(signal_power_linear / interference_power_linear))

        if not sir_values_db:
            if interference_free_ues:
                logger.info(
                    "All %d valid wideband SIR samples are interference-free; "
                    "returning +inf",
                    interference_free_ues
                )
                return float("inf")
            logger.warning(
                "No wideband SIR samples with a valid serving signal"
            )
            return 0.0

        sir_db = float(np.mean(sir_values_db))
        if interference_free_ues:
            logger.info(
                "Excluded %d interference-free UEs from the finite mean SIR",
                interference_free_ues
            )
        logger.info(f"Mean wideband SIR across {len(sir_values_db)} UEs: {sir_db:.2f} dB")
        return sir_db
    ...
    def _compute_serving_assignments(self, association_method: str = 'rsrp') -> Dict[int, int]:
        ...
        Args:
            association_method: Association method to use:
                - 'rsrp': RSRP-based (path loss, shadow fading, and BS gain)
                - 'cir': CIR power-based (legacy)
                - 'distance': Minimal distance-based
        ...
        if association_method == 'rsrp':
            return self._compute_rsrp_serving_assignments()
        ...
        for ue_idx in ue_indices:
            ...
            for link in ue_links:
                ...
                if association_method == 'distance':
    ...
    def _analyze_isac_calibration_paths(self) -> Optional[Dict[str, np.ndarray]]:
        """Calculate ISAC spreads from the paths used to synthesize the CIR."""
        if self.isac_channel_type not in ('target', 'background'):
            return None

        channel_group = (
            'isacTargetLinks'
            if self.isac_channel_type == 'target'
            else 'isacBackgroundLinks'
        )
        path_group_name = f'topology/{channel_group}/calibrationPaths'
        dataset_names = (
            'group_id', 'delay_ns', 'power', 'aod_deg', 'zod_deg',
            'aoa_deg', 'zoa_deg'
        )
        schema_name = (
            f'topology/{channel_group}/calibration_paths_schema_version'
        )

        with h5py.File(self.h5_file_path, 'r') as h5_file:
            if path_group_name not in h5_file:
                if schema_name in h5_file:
                    return {
                        name: np.array([])
                        for name in ('DS', 'ASD', 'ZSD', 'ASA', 'ZSA')
                    }
                return None
            path_group = h5_file[path_group_name]
            missing = [name for name in dataset_names if name not in path_group]
            if missing:
                raise ValueError(
                    f"Incomplete {path_group_name}: missing {', '.join(missing)}"
                )
            path_data = {
                name: np.asarray(path_group[name][:]) for name in dataset_names
            }

        path_count = len(path_data['group_id'])
        if any(len(values) != path_count for values in path_data.values()):
            raise ValueError(f"Inconsistent dataset lengths in {path_group_name}")

        results = {name: [] for name in ('DS', 'ASD', 'ZSD', 'ASA', 'ZSA')}
        for group_id in np.unique(path_data['group_id']):
            group_mask = path_data['group_id'] == group_id
            valid_mask = group_mask & np.isfinite(path_data['delay_ns'])
            valid_mask &= np.isfinite(path_data['power']) & (path_data['power'] > 0)
            for angle_name in ('aod_deg', 'zod_deg', 'aoa_deg', 'zoa_deg'):
                valid_mask &= np.isfinite(path_data[angle_name])

            if not np.any(valid_mask):
                logger.warning(
                    "ISAC calibration path group %s has no valid positive-power paths",
                    group_id
                )
                continue

            powers = path_data['power'][valid_mask].astype(float)
            delays_ns = path_data['delay_ns'][valid_mask].astype(float)
            total_power = np.sum(powers)
            mean_delay = np.sum(powers * delays_ns) / total_power
            delay_variance = np.sum(
                powers * np.square(delays_ns - mean_delay)
            ) / total_power
            results['DS'].append(np.sqrt(max(float(delay_variance), 0.0)))
            ...
            for metric_name, (dataset_name, is_zenith) in angle_metrics.items():
                results[metric_name].append(
                    self.compute_circular_angle_spread(
                        path_data[dataset_name][valid_mask].tolist(),
                        powers.tolist(),
                        is_zenith=is_zenith
                    )
                )
        ...
        logger.info(
            "Collected ISAC %s spreads from %d generated channel path groups",
            self.isac_channel_type,
            len(results['DS'])
        )
    ...
    def analyze_delay_and_angle_spreads_isac(self) -> Dict[str, np.ndarray]:
        ...
        calibration_path_spreads = self._analyze_isac_calibration_paths()
        if calibration_path_spreads is None:
            raise ValueError(
                "ISAC calibrationPaths are missing from the H5 file; "
                "regenerate it with the current gpu3gppchan before analyzing "
                "Phase-2 target/background spreads"
            )
        self._save_spreads_to_json(
            calibration_path_spreads, f'isac_{self.isac_channel_type}'
        )
        return calibration_path_spreads
    ...
    def plot_cdf(self, data: np.ndarray, title: str, xlabel: str, ylabel: str = 'CDF (%)', 
        ...
        plt.plot(sorted_data, cdf_values, linewidth=2.5, label=SIM_LABEL, color='blue', zorder=3)
    ...
    def print_serving_cell_assignments(self, serving_assignments: Dict[int, Dict[str, Any]],
                                       association_method: str = 'cir'):
        """
        Print serving cell assignments for visual verification.
        
        Args:
            serving_assignments: Rich or simple per-UE serving-cell assignments
            association_method: Association method used to produce the assignments
        """
        if not serving_assignments:
            print("No serving cell assignments available")
            return
        
        print(f"\n=== SERVING CELL ASSIGNMENTS ({association_method.upper()}-based) ===")
        ...
        for ue_idx in sorted(serving_assignments.keys()):
            ...
            n_sectors = self.system_level_config.n_sector_per_site if self.system_level_config else 3
            site_id = assignment.get('site_id', cell_id // n_sectors)
            sector_id = assignment.get('sector_id', cell_id % n_sectors)
            cir_power = assignment.get('cir_power', float('nan'))
            power_per_cell = assignment.get('power_per_cell', {})
            ...
            if ue_idx < 3 and power_per_cell:
    ...
    def compute_coupling_loss_serving_cells_only(self, use_virtualization: bool = False,
                                                 association_method: str = 'cir') -> np.ndarray:
        """
        Compute coupling loss for serving cell links only.
        Serving cells are determined by the requested association method.
        Coupling loss = Path loss - Shadow fading - BS Antenna gain
        
        Args:
            use_virtualization: If True, use antenna virtualization for gain calculation
            association_method: Association method used for serving-cell selection
    ...
    def _get_phase2_best_beam_powers(self) -> Dict[int, Dict[int, float]]:
        """Return cached per-UE/per-cell CIR power after best-of-12 TX beam selection."""
        if self._phase2_best_beam_powers is not None:
            return self._phase2_best_beam_powers

        powers_by_ue: Dict[int, Dict[int, float]] = {}
        beam_angles_deg = np.linspace(-60.0, 60.0, 12)

        for cell_id, cell_cir in self.cir_per_cell.items():
            cir = np.asarray(cell_cir)
            if cir.ndim == 4:
                # [UE, snapshot, TX, tap] -> add a singleton RX dimension.
                cir = cir[:, :, np.newaxis, :, :]
            if cir.ndim != 5:
                logger.warning(
                    "Cell %d CIR shape %s cannot be used for Phase-2 beam selection",
                    cell_id,
                    cir.shape,
                )
                continue

            n_ues, _, _, n_tx, n_taps = cir.shape
            cell = next((item for item in self.cell_params if item.cid == cell_id), None)
            panel = self.antenna_panels.get(cell.ant_panel_idx) if cell else None
            if panel is None or len(panel.ant_size) < 5:
                logger.warning(
                    "Cell %d lacks antenna-panel dimensions for Phase-2 beam selection",
                    cell_id,
                )
                continue
            _, _, n_vertical, n_horizontal, n_polarizations = panel.ant_size
            if n_vertical * n_horizontal * n_polarizations != n_tx:
                logger.warning(
                    "Cell %d antenna dimensions M=%d, N=%d, P=%d do not match "
                    "%d CIR TX elements",
                    cell_id,
                    n_vertical,
                    n_horizontal,
            ...
            element_indices = np.arange(n_vertical)
            beam_weights = np.asarray([
                np.exp(1j * np.pi * np.cos(np.deg2rad(angle)) * element_indices)
                / np.sqrt(n_vertical)
                for angle in beam_angles_deg
            ], dtype=np.complex64)

            # Antenna index = (m * N + n) * P + p.  A virtual port fixes
            # horizontal position n and polarization p, then combines M.
            cir_panel = cir.reshape(
                n_ues,
                cir.shape[1],
                cir.shape[2],
                n_vertical,
            ...
            beamformed = np.einsum(
                'usrmnpl,bm->usrnpbl',
                cir_panel,
                np.conj(beam_weights),
                optimize=True,
            )

            ntaps = np.full(n_ues, n_taps, dtype=np.int64)
            if cell_id in self.cir_ntaps_per_cell:
                stored_ntaps = np.asarray(
                    self.cir_ntaps_per_cell[cell_id]
                ).reshape(-1)
                count = min(n_ues, len(stored_ntaps))
                ntaps[:count] = np.clip(stored_ntaps[:count], 0, n_taps)
            valid_taps = np.arange(n_taps)[np.newaxis, :] < ntaps[:, np.newaxis]
            beamformed *= valid_taps[
                :, np.newaxis, np.newaxis, np.newaxis,
                np.newaxis, np.newaxis, :
            ]

            # Sum snapshots, UE RX branches, and taps; select the strongest
            # virtual port and beam independently for each candidate cell.
            beam_powers = np.sum(np.abs(beamformed) ** 2, axis=(1, 2, 6))
            best_powers = np.max(beam_powers, axis=(1, 2, 3))

            row_to_ue = {
                row_idx: ue_id
                for ue_id, row_idx in self.ue_row_mappings.get(cell_id, {}).items()
            }
            if not row_to_ue:
                row_to_ue = {
                    row_idx: active_link.uid
                    for row_idx, active_link in enumerate(
                        link for link in self.active_link_params
                        if link.cid == cell_id
                    )
                }

            for row_idx, power in enumerate(best_powers):
                ue_id = row_to_ue.get(row_idx)
                if ue_id is not None:
                    powers_by_ue.setdefault(int(ue_id), {})[int(cell_id)] = float(power)

        self._phase2_best_beam_powers = powers_by_ue
        return powers_by_ue
    ...
    def _apply_virtualization_to_cir(self, H, calibration_phase, tilt_deg=12.0, sum_ports=False) -> float:
        ...
        if calibration_phase == 1:
            ...
            # Phase 2: Group one polarization's eight-element vertical column
            # into a virtualized port: 64 antennas -> 8 TX ports (64/8 = 8).
            n_elements_per_port = 8
            ...
            for port_idx in range(n_tx_ports):
                ...
                H_port = H[:, start_idx:end_idx, :]  # Shape: (n_snapshot, 8, ntaps)
    ...
    def compute_sir_sinr_geometry(self, center_freq_hz: float = 6e9, association_method: str = 'cir',
        ...
        **CIR-BASED APPROACH** (includes all channel effects):
        ...
        Args:
            ...
            association_method: Association method to use:
                - 'rsrp': RSRP-based (pathloss, shadow fading, and BS gain)
        ...
        if serving_assignments and isinstance(next(iter(serving_assignments.values())), int):
            ...
            for ue_idx, cell_id in serving_assignments.items():
                ...
                complete_beam_powers = (
                    self._get_complete_phase2_best_beam_powers(
                        best_beam_powers, ue_idx
                    )
                )
                if complete_beam_powers is not None:
                    full_assignments[ue_idx] = {
                        'cell_id': cell_id,
                        'power_per_cell': complete_beam_powers.copy()
                    }
                    continue
    ...
    def generate_all_plots(self, output_dir: str = "channel_analysis_plots", association_method: str = 'rsrp',
        ...
        print(f"{'='*60}")
        if association_method == 'rsrp':
            print("Using RSRP-based association (pathloss, shadow fading, and BS gain)")
        ...
        if use_virtualization:
            ...
            if self.calibration_phase == 1:
                ...
                print("Phase 2: 8 elements/port grouping, angle sweep [-60°, 60°], 12 angles")
        else:
        ...
        serving_coupling_losses = self.compute_coupling_loss_serving_cells_only(
            use_virtualization, association_method
        )
        ...
        if not is_isac_mode:
            sir_db = self.compute_wideband_sir(association_method=association_method)
        ...
        serving_assignments = self._compute_serving_assignments(association_method)
        printable_assignments = {
            ue_idx: assignment if isinstance(assignment, dict) else {'cell_id': assignment}
            for ue_idx, assignment in serving_assignments.items()
        }
        self.print_serving_cell_assignments(printable_assignments, association_method)
        ...
        # 5b. Pathloss CDF for serving cells only
        serving_coupling_losses = self.compute_coupling_loss_serving_cells_only(
            use_virtualization, association_method
        )
        if len(serving_coupling_losses) > 0:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            '  Phase 2: Groups 8 elements into 1 CSI-RS port (64 elements -> 8 TX ports)\n'
            '           Sweeps 12 angles from -60° to 60° (10° steps) relative to sector orientation\n'
            '           Uses the equivalent idealized-port response and selects the best angle\n'
    ...
    parser.add_argument('--backend', choices=['gpu', 'cpu'], default='gpu',
                        help="Simulation backend shown in plot legends: 'Simulation GPU' (default) or 'Simulation CPU'")
    ...
    global SIM_LABEL
    SIM_LABEL = f"Simulation {args.backend.upper()}"
    ...
    if args.multi_seed:
        ...
        if len(h5_files) > 1:
            ...
            for h5_file in h5_files:
                ...
                coupling_losses = temp_analyzer.compute_coupling_loss_serving_cells_only(
                    args.use_virtualization, association_method
                )
