...
SEED_START=0
SEED_END=19
YAML_CONFIG="testBenches/gpu3GPPChan/config/statistic_channel_config_isac_phase2.yaml"
DATASET_NAME="isac_uav_phase2_target"
REFERENCE_JSON="testBenches/gpu3GPPChan/util/3gpp_calibration_isac_uav_phase2.json"
CALIBRATION_PHASE=2
...
# SLS_CHAN_EX may be overridden from the environment (e.g. by the internal CICD
# harness to point at an alternate build such as TR38901_REL18_PARAMS=ON).
SLS_CHAN_EX="${SLS_CHAN_EX:-$SDK_ROOT/build.aarch64/testBenches/gpu3GPPChan/examples/sls_chan/sls_chan_ex}"
ANALYSIS_SCRIPT="$SDK_ROOT/testBenches/gpu3GPPChan/util/analysis_channel_stats.py"
# Extra arguments appended to the analysis invocation (e.g. "--log-level INFO
# --log-file <dir>/analysis.log" so KS statistics are captured). Default: none.
...
# Word-split ANALYSIS_EXTRA_ARGS into an array (read splits on IFS but does not
# glob, so a stray *, ?, or [] in the args cannot expand against the CWD)
read -r -a analysis_extra_args <<< "$ANALYSIS_EXTRA_ARGS"
