...
bool PMUReader::addCounter(unsigned int type, unsigned long long config, bool disable = false) {
    ...
    if(!disable) {
        ...
        current_fid = static_cast<int>(
            syscall(__NR_perf_event_open, &pea, 0, -1, trigger_fid, 0));
        if (current_fid == -1) {
          ...
          // Store counter id (not needed for current implementation, but this
          // can be used if
...
int PMUReader::readCounters(uint64_t* counters, int num_slots) {
    ...
    if(num_counters_ > 0) {
        ...
        int read_index = 0;
        for(int ii = 0; ii < static_cast<int>(data.num_counters) && ii<num_slots; ii++) {
            if(fids_[ii] == -1) {
    ...
    return static_cast<int>(data.num_counters);
}
...
void PMUReaderTopdown::readMetrics(uint64_t* counts, TopdownMetrics* metrics) {
    ...
    double CPU_CYCLES = static_cast<double>(counts[0]);
    double OP_SPEC = static_cast<double>(counts[1]);
    double OP_RETIRED = static_cast<double>(counts[2]);
    double BR_MIS_PRED = static_cast<double>(counts[3]);
    double STALL_SLOT = static_cast<double>(counts[4]);
    double STALL_SLOT_FRONTEND = static_cast<double>(counts[5]);
    double STALL_SLOT_BACKEND = static_cast<double>(counts[6]);
...
void PMUReaderCacheMetrics::readMetrics(uint64_t* counts, CacheMetrics* metrics) {
    ...
    double CPU_CYCLES = static_cast<double>(counts[0]);
    double INST_RETIRED = static_cast<double>(counts[1]);
    double MEM_ACCESS = static_cast<double>(counts[2]);
    double L3D_CACHE_REFILL = static_cast<double>(counts[3]);
    double L2D_CACHE_REFILL = static_cast<double>(counts[4]);
    double L1D_CACHE_REFILL = static_cast<double>(counts[5]);
    double L1I_CACHE_REFILL = static_cast<double>(counts[6]);
...
void PMUReaderCacheMissRatios::readMetrics(uint64_t* counts, CacheMissRatioMetrics* metrics) {
    ...
    [[maybe_unused]] double CPU_CYCLES = static_cast<double>(counts[0]);
    double L3D_CACHE_REFILL = static_cast<double>(counts[1]);
    double L3D_CACHE = static_cast<double>(counts[2]);
    double L2D_CACHE_REFILL = static_cast<double>(counts[3]);
    double L2D_CACHE = static_cast<double>(counts[4]);
    double L1D_CACHE_REFILL = static_cast<double>(counts[5]);
    double L1D_CACHE = static_cast<double>(counts[6]);
...
{
    const size_t out_sz = static_cast<size_t>(max_chars);

    //Write empty string (assuming pmu metrics are disabled)
    snprintf(out_str, out_sz, "%i", pmu_type_);
    ...
    if(pmu_type_ == PMU_TYPE_GENERAL) {
        ...
        snprintf(out_str, out_sz,
                 "%i,%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64,
                 pmu_type_,
                 metrics.cycle_count,
                 metrics.instr_count,
    ...
    if(pmu_type_ == PMU_TYPE_TOPDOWN) {
        ...
        snprintf(out_str, out_sz, "%i,%.2f,%.2f,%.2f,%.2f",
                 pmu_type_,
                 metrics.retiring,
        ...
        snprintf(out_str, out_sz, "%i,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f",
                 pmu_type_,
