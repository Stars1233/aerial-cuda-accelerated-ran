...
#include "nvlog.h"
#define TAG (NVLOG_TAG_BASE_UTIL + 137)  // same value as legacy NVLOG_TAG_BASE_CUPHY_DRIVER + 17 ("DRV.FUNC")
...
TaskInstrumentation::TaskInstrumentation(
    ...
    const int max_subtasks) noexcept
    ...
    if (tracing_mode_ == TracingMode::START_END_ONLY) {
        ...
        *p++ = 's';
        *p++ = ':';
        char* new_p = perf_metrics::fmt_u64toa_safe(
            static_cast<uint64_t>(perf_metrics::nowNs().count()), p, end);
        if (new_p != nullptr) {
            p = new_p;
        }
        *p = '\0';
...
void TaskInstrumentation::add(const char* subtask_name) noexcept {
    ...
    [[maybe_unused]] auto ret = formatSubtask(subtask_name, abs_time_ns); 
    
}

void TaskInstrumentation::append(const char* subtask_name, perf_metrics::t_ns time) noexcept {
    if (tracing_mode_ == TracingMode::FULL_TRACING) {
        [[maybe_unused]] auto ret = formatSubtask(subtask_name, static_cast<uint64_t>(time.count()));
    }
}
...
bool TaskInstrumentation::formatSubtask(const char* subtask_name, uint64_t timestamp_ns) noexcept {
    ...
    *p++ = ',';
    ...
    current_offset_ = static_cast<uint32_t>(p - subtask_results_);
    ++subtask_count_;
    return true;
}
