 ...
 *
 * Build: cmake with ENABLE_TESTS=ON, then ninja test_u64toa_perf
 * Run:   ./test_u64toa_perf (from CMake build directory)
...
void benchmark_fmt_u64toa_safe(const std::vector<uint64_t>& test_values,
    ...
    for (int i = 0; i < config.warmup_iterations; ++i) {
        uint64_t value = test_values[static_cast<size_t>(i) % test_values.size()];
...
void benchmark_sprintf(const std::vector<uint64_t>& test_values,
    ...
    for (int i = 0; i < config.test_iterations; ++i) {
        ...
        timer.start();
        int result = sprintf(buffer, "%" PRIu64, value);
        uint64_t elapsed = timer.stop_ns();
