...
#if !defined(SCF_5G_FAPI_PUCCH234_PDU_PARSER_HPP_INCLUDED_)
#define SCF_5G_FAPI_PUCCH234_PDU_PARSER_HPP_INCLUDED_
...
/**
 * Parses a single PUCCH format 2/3/4 PDU from a UL_TTI.request message.
 *
 * Uses the same scf_fapi_pucch_pdu_t struct as PucchPduParser;
 * the format_type field in the PDU distinguishes format 0/1 from 2/3/4
 * at runtime.  Dispatched via UL_TTI_PDU_TYPE_PUCCH_2_3_4.
 *
 * @tparam V  Type satisfying UlModuleView.
 */
template<UlModuleView V>
class Pucch234PduParser final
{
public:
    using pdu_t = scf_fapi_pucch_pdu_t;
    static constexpr scf_fapi_ul_tti_pdu_type_t pdu_type = UL_TTI_PDU_TYPE_PUCCH_2_3_4;

    explicit Pucch234PduParser(V& v) noexcept : view_{&v} {}
    ...
    /**
     * Process one PUCCH format 2/3/4 PDU for the given sfn/slot.
     *
     * @param[in] sfn   System Frame Number.
     * @param[in] slot  Slot number.
     * @param[in] pdu   PUCCH format 2/3/4 PDU to process.
...
template<UlModuleView V>
bool Pucch234PduParser<V>::parse(uint16_t sfn, uint16_t slot,
                                  const pdu_t& pdu) noexcept
{
    if (pdu.format_type != UL_TTI_PUCCH_FORMAT_2 &&
        pdu.format_type != UL_TTI_PUCCH_FORMAT_3 &&
        pdu.format_type != UL_TTI_PUCCH_FORMAT_4)
    {
        return false;
    }
    if (!cell_context_ready_)
    {
        NVLOGE_FMT(detail::k_tag, AERIAL_L2ADAPTER_EVENT,
                   "Pucch234PduParser: setup_cell was not called before parse sfn={} slot={}",
                   sfn, slot);
        return false;
    }

    return detail::parse_pucch_pdu_common(*view_, local_cell_id_, sfn, slot, pdu, "Pucch234PduParser");
}

} // namespace scf_5g_fapi

#endif // SCF_5G_FAPI_PUCCH234_PDU_PARSER_HPP_INCLUDED_
