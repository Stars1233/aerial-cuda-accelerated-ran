...
namespace
{
    ...
    // structure.
    //template <int BG> using app_loc_t = app_loc_address_p_desc<__half2, BG>;
    template <int BG> using app_loc_t = app_loc_address_p_nzs_desc<__half2, BG>;
    ...
    struct ldpc2_split_index_p_x2_desc_dyn_kernel_config
    {
    ...
    int get_tb_shmem_required(int BG, int num_parity_nodes, int Z)
    {
        return static_cast<int>(shmem_size_with_et_context<ldpc_et_context_x2_t>(
            static_cast<uint32_t>(get_shmem_required(BG, num_parity_nodes, Z))));
    }
#if LDPC_DECODE_USE_TB_SCAN
    //------------------------------------------------------------------
    // get_token_addr()
...
////////////////////////////////////////////////////////////////////////
// ldpc2_BG1_split_index_p_x2_desc_dyn()
// Kernel for base graph 1 (legacy tensor interface)
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG1_split_index_p_x2_desc_dyn(LDPC_kernel_params params, app_loc_t<1>::bg_desc_t bgdesc)
{
    ...
    typedef ldpc2_split_index_p_x2_desc_dyn_kernel_config<1,                                          // BG
...
////////////////////////////////////////////////////////////////////////
// ldpc2_BG2_split_index_p_x2_desc_dyn()
// Kernel for base graph 1 (legacy tensor interface)
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG2_split_index_p_x2_desc_dyn(LDPC_kernel_params params, app_loc_t<2>::bg_desc_t bgdesc)
{
    // Shared memory is allocated dynamically
    extern __shared__ char smem[];
    ...
    //------------------------------------------------------------------
    // Kernel configuration template
    typedef ldpc2_split_index_p_x2_desc_dyn_kernel_config<2,                                          // BG
                                                          ldpc2::LDPC_kernel_params> kernel_config_t; // params struct
...
////////////////////////////////////////////////////////////////////////
// ldpc2_BG1_split_index_p_x2_desc_dyn_tb()
// Kernel for base graph 1 (transport block interface)
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG1_split_index_p_x2_desc_dyn_tb(cuphyLDPCDecodeDesc_t decodeDesc, app_loc_t<1>::bg_desc_t bgdesc)
{
    ...
    kernel_config_t::sched_t sched(smem,
                                   ...
                                   static_cast<int>(__cvta_generic_to_shared(smem)),
                                   threadIdx.x);
    const uint32_t size_before_et = static_cast<uint32_t>(get_shmem_required(
        kernel_config_t::BG, decodeDesc.config.num_parity_nodes, decodeDesc.config.Z));
...
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG1_split_index_p_x2_desc_dyn_tb_no_accessories(cuphyLDPCDecodeDesc_t decodeDesc, app_loc_t<1>::bg_desc_t bgdesc)
{
...
////////////////////////////////////////////////////////////////////////
// ldpc2_BG2_split_index_p_x2_desc_dyn_tb()
// Kernel for base graph 2 (transport block interface)
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG2_split_index_p_x2_desc_dyn_tb(cuphyLDPCDecodeDesc_t decodeDesc, app_loc_t<2>::bg_desc_t bgdesc)
{
    [[maybe_unused]] constexpr bool ENABLE_ACCESSORY_FEATURES = true;
    extern __shared__ char smem[];
    // Shared memory is allocated dynamically
...
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG2_split_index_p_x2_desc_dyn_tb_no_accessories(cuphyLDPCDecodeDesc_t decodeDesc, app_loc_t<2>::bg_desc_t bgdesc)
{
    [[maybe_unused]] constexpr bool ENABLE_ACCESSORY_FEATURES = false;
    extern __shared__ char smem[];
    // Shared memory is allocated dynamically
...
////////////////////////////////////////////////////////////////////////
// split_index_p_x2_desc_dyn::decode()
cuphyStatus_t split_index_p_x2_desc_dyn::decode(ldpc::decoder&                     dec,
                                                LDPC_output_t&                     tDst,
                                                ...
                                                cudaStream_t                       strm)
{
    DEBUG_PRINTF("ldpc::decode_ldpc2_split_index_p_x2_desc_dyn()\n");
    //------------------------------------------------------------------
        ...
        case 1:
            {
                ...
                DEBUG_PRINT_FUNC_MAX_BLOCKS(ldpc2_BG1_split_index_p_x2_desc_dyn, blkDim, SHMEM_SIZE);

                //------------------------------------------------------------------
                // Launch the kernel
                ldpc2_BG1_split_index_p_x2_desc_dyn<<<grdDim, blkDim, SHMEM_SIZE, strm>>>(params, *bgdesc);
        ...
        case 2:
            {
                ...
                DEBUG_PRINT_FUNC_MAX_BLOCKS(ldpc2_BG2_split_index_p_x2_desc_dyn, blkDim, SHMEM_SIZE);

                //------------------------------------------------------------------
                // Launch the kernel
                ldpc2_BG2_split_index_p_x2_desc_dyn<<<grdDim, blkDim, SHMEM_SIZE, strm>>>(params, *bgdesc);
                s = CUPHY_STATUS_SUCCESS;
            }
            break;
        default:
...
////////////////////////////////////////////////////////////////////////
// split_index_p_x2_desc_dyn::decode_tb()
cuphyStatus_t split_index_p_x2_desc_dyn::decode_tb(ldpc::decoder&               dec,
                                                   const cuphyLDPCDecodeDesc_t& decodeDesc,
                                                   cudaStream_t                 strm)
{
    DEBUG_PRINTF("ldpc2::split_index_p_x2_desc_dyn::decode_tb()\n");
    ...
    if(decodeDesc.config.llr_type == CUPHY_R_16F)
    {
        ...
        //------------------------------------------------------------------
        // Determine the dynamic amount of shared memory
        const uint32_t SHMEM_SIZE = get_tb_shmem_required(decodeDesc.config.BG,
        ...
        case 1:
            {
                ...
                DEBUG_PRINT_FUNC_MAX_BLOCKS(ldpc2_BG1_split_index_p_x2_desc_dyn_tb, blkDim, SHMEM_SIZE);

                //------------------------------------------------------------------
                // Launch the kernel
               LDPC_LAUNCH_TB_KERNEL_X2(ldpc2_BG1_split_index_p_x2_desc_dyn_tb, decodeDesc.config.flags,
        ...
        case 2:
            {
                ...
                DEBUG_PRINT_FUNC_MAX_BLOCKS(ldpc2_BG2_split_index_p_x2_desc_dyn_tb, blkDim, SHMEM_SIZE);

                //------------------------------------------------------------------
                // Launch the kernel
                LDPC_LAUNCH_TB_KERNEL_X2(ldpc2_BG2_split_index_p_x2_desc_dyn_tb, decodeDesc.config.flags,
                    grdDim, blkDim, SHMEM_SIZE, strm, decodeDesc, *bgdesc);
                s = CUPHY_STATUS_SUCCESS;
            }
            break;
...
////////////////////////////////////////////////////////////////////////
// split_index_p_x2_desc_dyn::get_workspace_size()
std::pair<bool, size_t> split_index_p_x2_desc_dyn::get_workspace_size(const ldpc::decoder&               dec,
                                                                      const cuphyLDPCDecodeConfigDesc_t& config,
                                                                      int                                num_cw)
{
...
////////////////////////////////////////////////////////////////////////
// split_index_p_x2_desc_dyn::split_index_p_x2_desc_dyn()
split_index_p_x2_desc_dyn::split_index_p_x2_desc_dyn(ldpc::decoder& dec)
{
    //------------------------------------------------------------------
    // Determine the maximum amount of shared memory that could be used
    // by a kernel
    const int MAX_BG1_SHMEM_SIZE = static_cast<int>(get_tb_shmem_required(1,                          // BG
                                                                       max_num_parity<1>::value,      // max parity nodes
                                                                       CUPHY_LDPC_MAX_LIFTING_SIZE)); // lifting size
    const int MAX_BG2_SHMEM_SIZE = static_cast<int>(get_tb_shmem_required(2,                          // BG
    ...
    {
        func_attr_t((const void*)ldpc2_BG1_split_index_p_x2_desc_dyn,    std::min(MAX_BG1_SHMEM_SIZE, MAX_SHMEM)),
        func_attr_t((const void*)ldpc2_BG2_split_index_p_x2_desc_dyn,    std::min(MAX_BG2_SHMEM_SIZE, MAX_SHMEM)),
        func_attr_t((const void*)ldpc2_BG1_split_index_p_x2_desc_dyn_tb, std::min(MAX_BG1_SHMEM_SIZE, MAX_SHMEM)),
        func_attr_t((const void*)ldpc2_BG1_split_index_p_x2_desc_dyn_tb_no_accessories, std::min(MAX_BG1_SHMEM_SIZE, MAX_SHMEM)),
        func_attr_t((const void*)ldpc2_BG2_split_index_p_x2_desc_dyn_tb, std::min(MAX_BG2_SHMEM_SIZE, MAX_SHMEM)),
        func_attr_t((const void*)ldpc2_BG2_split_index_p_x2_desc_dyn_tb_no_accessories, std::min(MAX_BG2_SHMEM_SIZE, MAX_SHMEM))
    };
    ...
    //------------------------------------------------------------------
    DEBUG_PRINT_FUNC_ATTRIBUTES(ldpc2_BG1_split_index_p_x2_desc_dyn);
    DEBUG_PRINT_FUNC_ATTRIBUTES(ldpc2_BG2_split_index_p_x2_desc_dyn);
    DEBUG_PRINT_FUNC_ATTRIBUTES(ldpc2_BG1_split_index_p_x2_desc_dyn_tb);
    DEBUG_PRINT_FUNC_ATTRIBUTES(ldpc2_BG2_split_index_p_x2_desc_dyn_tb);
}

////////////////////////////////////////////////////////////////////////
// split_index_p_x2_desc_dyn::can_decode_config()
bool split_index_p_x2_desc_dyn::can_decode_config(const ldpc::decoder&               dec,
                                                  const cuphyLDPCDecodeConfigDesc_t& cfg)
{
    ...
    const uint32_t SHMEM_BYTES =
        shmem_size_for_selected_kernel<ldpc_et_context_x2_t>(
            get_tb_shmem_required(cfg.BG, cfg.num_parity_nodes, cfg.Z),
            cfg.flags);
...
////////////////////////////////////////////////////////////////////////
// split_index_p_x2_desc_dyn::get_launch_config()
cuphyStatus_t split_index_p_x2_desc_dyn::get_launch_config(const ldpc::decoder&           dec,
                                                           cuphyLDPCDecodeLaunchConfig_t& launchConfig)
{
    ...
    const uint32_t SHMEM_SIZE = get_tb_shmem_required(launchConfig.decode_desc.config.BG,
    ...
    cudaError_t    e = (BG == 1) ? LDPC_GET_TB_KERNEL_FUNCTION(deviceFunction, ldpc2_BG1_split_index_p_x2_desc_dyn_tb, launchConfig.decode_desc.config.flags) :
                                   LDPC_GET_TB_KERNEL_FUNCTION(deviceFunction, ldpc2_BG2_split_index_p_x2_desc_dyn_tb, launchConfig.decode_desc.config.flags);
    if (e != cudaSuccess)
    {
