...
namespace
{
    ...
    template <int   BG_,
              ...
              int   MAX_PARITY_ROWS,
              class TKernelParams>
    struct ldpc2_reg_box_plus_spec_kernel_config
    {
...
////////////////////////////////////////////////////////////////////////
// ldpc2_BG1_reg_box_plus_spec()
// Base graph 1 kernel, "legacy" tensor interface
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG1_reg_box_plus_spec(LDPC_kernel_params params, null_BG_desc_t<1> bgdesc)
{
    ...
    typedef ldpc2_reg_box_plus_spec_kernel_config<1,
...
////////////////////////////////////////////////////////////////////////
// ldpc2_BG2_reg_box_plus_spec()
// Base graph 2 kernel, "legacy" tensor interface
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG2_reg_box_plus_spec(LDPC_kernel_params params, null_BG_desc_t<2> bgdesc)
{
    // Shared memory is allocated dynamically
    extern __shared__ char smem[];
    ...
    //------------------------------------------------------------------
    // Kernel configuration template
    typedef ldpc2_reg_box_plus_spec_kernel_config<2, // BG
...
////////////////////////////////////////////////////////////////////////
// ldpc2_BG1_reg_box_plus_spec_tb()
// Base graph 1 kernel, transport block interface
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG1_reg_box_plus_spec_tb(cuphyLDPCDecodeDesc_t decodeDesc, null_BG_desc_t<1> bgdesc)
{
...
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG1_reg_box_plus_spec_tb_no_accessories(cuphyLDPCDecodeDesc_t decodeDesc, null_BG_desc_t<1> bgdesc)
{
...
////////////////////////////////////////////////////////////////////////
// ldpc2_BG2_reg_box_plus_spec_tb()
// Base graph 2 kernel, transport block interface
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG2_reg_box_plus_spec_tb(cuphyLDPCDecodeDesc_t decodeDesc, null_BG_desc_t<2> bgdesc)
{
    [[maybe_unused]] constexpr bool ENABLE_ACCESSORY_FEATURES = true;
    extern __shared__ char smem[];
    // Shared memory is allocated dynamically
...
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG2_reg_box_plus_spec_tb_no_accessories(cuphyLDPCDecodeDesc_t decodeDesc, null_BG_desc_t<2> bgdesc)
{
    [[maybe_unused]] constexpr bool ENABLE_ACCESSORY_FEATURES = false;
    extern __shared__ char smem[];
    // Shared memory is allocated dynamically
...
////////////////////////////////////////////////////////////////////////
// reg_box_plus_spec::decode()
cuphyStatus_t reg_box_plus_spec::decode(ldpc::decoder&                     dec,
                                        LDPC_output_t&                     tDst,
                                        ...
                                        cudaStream_t                       strm)
{
    DEBUG_PRINTF("ldpc2::reg_box_plus_spec::decode()\n");
    //------------------------------------------------------------------
    ...
    if((llrType == CUPHY_R_16F) &&
       ...
       (((config.BG == 1) && (config.num_parity_nodes <= max_parity_nodes<1>::value)) ||
        ((config.BG == 2) && (config.num_parity_nodes <= max_parity_nodes<2>::value))))
    {
        switch(config.BG)
        {
        case 1:
            {
                DEBUG_PRINT_FUNC_MAX_BLOCKS(ldpc2_BG1_reg_box_plus_spec, blkDim, SHMEM_SIZE);

                //-  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -
                // Launch the kernel
                ldpc2_BG1_reg_box_plus_spec<<<grdDim, blkDim, SHMEM_SIZE, strm>>>(params, null_BG_desc_t<1>{});
        ...
        case 2:
            {
                DEBUG_PRINT_FUNC_MAX_BLOCKS(ldpc2_BG2_reg_box_plus_spec, blkDim, SHMEM_SIZE);

                //-  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -
                // Launch the kernel
                ldpc2_BG2_reg_box_plus_spec<<<grdDim, blkDim, SHMEM_SIZE, strm>>>(params, null_BG_desc_t<2>{});
                s = CUPHY_STATUS_SUCCESS;
            }
            break;
        default:
...
////////////////////////////////////////////////////////////////////////
// reg_box_plus_spec::decode_tb()
cuphyStatus_t reg_box_plus_spec::decode_tb(ldpc::decoder&               dec,
                                           const cuphyLDPCDecodeDesc_t& decodeDesc,
                                           cudaStream_t                 strm)
{
    DEBUG_PRINTF("ldpc2::reg_box_plus_spec::decode_tb()\n");
    cuphyStatus_t s = CUPHY_STATUS_NOT_SUPPORTED;
    //------------------------------------------------------------------
    ...
    if((decodeDesc.config.llr_type == CUPHY_R_16F) &&
       ...
       (((decodeDesc.config.BG == 1) && (decodeDesc.config.num_parity_nodes <= max_parity_nodes<1>::value)) ||
        ((decodeDesc.config.BG == 2) && (decodeDesc.config.num_parity_nodes <= max_parity_nodes<2>::value))))
    {
        ...
        case 1:
            {
                ...
                DEBUG_PRINT_FUNC_MAX_BLOCKS(ldpc2_BG1_reg_box_plus_spec_tb, blkDim, SHMEM_SIZE);

                //------------------------------------------------------------------
                // Launch the kernel
                LDPC_LAUNCH_TB_KERNEL(ldpc2_BG1_reg_box_plus_spec_tb, decodeDesc.config.flags,
        ...
        case 2:
            {
                ...
                DEBUG_PRINT_FUNC_MAX_BLOCKS(ldpc2_BG2_reg_box_plus_spec_tb, blkDim, SHMEM_SIZE);

                //------------------------------------------------------------------
                // Launch the kernel
                LDPC_LAUNCH_TB_KERNEL(ldpc2_BG2_reg_box_plus_spec_tb, decodeDesc.config.flags,
...
////////////////////////////////////////////////////////////////////////
// reg_box_plus_spec::get_workspace_size()
std::pair<bool, size_t> reg_box_plus_spec::get_workspace_size(const ldpc::decoder&               dec,
                                                              const cuphyLDPCDecodeConfigDesc_t& config,
                                                              int                                num_cw)
{
...
////////////////////////////////////////////////////////////////////////
// reg_box_plus_spec::reg_box_plus_spec()
reg_box_plus_spec::reg_box_plus_spec(ldpc::decoder& desc)
{
    ...
    {
        func_attr_t((const void*)ldpc2_BG1_reg_box_plus_spec,    MAX_BG1_SHMEM_SIZE),
        func_attr_t((const void*)ldpc2_BG2_reg_box_plus_spec,    MAX_BG2_SHMEM_SIZE),
        func_attr_t((const void*)ldpc2_BG1_reg_box_plus_spec_tb, MAX_BG1_SHMEM_SIZE),
        func_attr_t((const void*)ldpc2_BG1_reg_box_plus_spec_tb_no_accessories, MAX_BG1_SHMEM_SIZE),
        func_attr_t((const void*)ldpc2_BG2_reg_box_plus_spec_tb, MAX_BG2_SHMEM_SIZE),
        func_attr_t((const void*)ldpc2_BG2_reg_box_plus_spec_tb_no_accessories, MAX_BG2_SHMEM_SIZE)
    };
    ...
    //------------------------------------------------------------------
    DEBUG_PRINT_FUNC_ATTRIBUTES(ldpc2_BG1_reg_box_plus_spec);
    DEBUG_PRINT_FUNC_ATTRIBUTES(ldpc2_BG2_reg_box_plus_spec);
    DEBUG_PRINT_FUNC_ATTRIBUTES(ldpc2_BG1_reg_box_plus_spec_tb);
    DEBUG_PRINT_FUNC_ATTRIBUTES(ldpc2_BG2_reg_box_plus_spec_tb);
}

////////////////////////////////////////////////////////////////////////
// reg_box_plus_spec::can_decode_config()
bool reg_box_plus_spec::can_decode_config(const ldpc::decoder&               dec,
...
////////////////////////////////////////////////////////////////////////
// reg_box_plus_spec::get_launch_config()
cuphyStatus_t reg_box_plus_spec::get_launch_config(const ldpc::decoder&           dec,
                                                   cuphyLDPCDecodeLaunchConfig_t& launchConfig)
{
    ...
    cudaError_t    e = (BG == 1) ?  LDPC_GET_TB_KERNEL_FUNCTION(deviceFunction, ldpc2_BG1_reg_box_plus_spec_tb, launchConfig.decode_desc.config.flags):
                                    LDPC_GET_TB_KERNEL_FUNCTION(deviceFunction, ldpc2_BG2_reg_box_plus_spec_tb, launchConfig.decode_desc.config.flags);
    if (e != cudaSuccess)
    {
