...
namespace
{
    ...
    template <int   BG_,
              ...
              int   MAX_PARITY_ROWS,
              class TKernelParams>
    struct ldpc2_reg_box_plus_fp_kernel_config
    {
...
////////////////////////////////////////////////////////////////////////
// ldpc2_BG1_reg_box_plus_fp()
// Base graph 1 kernel, "legacy" tensor interface
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG1_reg_box_plus_fp(LDPC_kernel_params params, app_loc_t<1>::bg_desc_t bgdesc)
{
    ...
    typedef ldpc2_reg_box_plus_fp_kernel_config<1,
    ...
    //const __half* app_smem = reinterpret_cast<const __half*>(smem);
...
////////////////////////////////////////////////////////////////////////
// ldpc2_BG2_reg_box_plus_fp()
// Base graph 2 kernel, "legacy" tensor interface
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG2_reg_box_plus_fp(LDPC_kernel_params params, app_loc_t<2>::bg_desc_t bgdesc)
{
    // Shared memory is allocated dynamically
    extern __shared__ char smem[];
    ...
    //------------------------------------------------------------------
    // Kernel configuration template
    typedef ldpc2_reg_box_plus_fp_kernel_config<2, // BG
...
////////////////////////////////////////////////////////////////////////
// ldpc2_BG1_reg_box_plus_fp_tb_impl<ENABLE_ACCESSORY_FEATURES, DUMP>()
// Base graph 1 body, shared by _tb, _tb_no_accessories and _tb_dumpApp.
// DUMP adds the ET-free per-iteration APP dump; discarded when false.
template <bool ENABLE_ACCESSORY_FEATURES, bool DUMP>
__device__ __forceinline__
void ldpc2_BG1_reg_box_plus_fp_tb_impl(const cuphyLDPCDecodeDesc_t& decodeDesc, app_loc_t<1>::bg_desc_t bgdesc)
{
    // smem_tb, not smem: a template's extern __shared__ clashes (nvcc #1556)
    // with the non-template kernels in this file.
    extern __shared__ char smem_tb[];
    ...
    kernel_config_t::llr_loader_t::load_sync(smem_tb, decodeDesc, blockIdx.x);

    kernel_config_t::sched_t sched(decodeDesc.config,
                                   bgdesc,
                                   static_cast<int>(__cvta_generic_to_shared(smem_tb)),
                                   threadIdx.x);
    auto    crc  = ldpc2::et_crc_traits<kernel_config_t::app_buf_t>::failure;
    int32_t iter = 0;
    if constexpr(DUMP)
    {
        for(iter = 0; iter < decodeDesc.config.max_iterations; ++iter)
        {
            sched.do_iteration();
            ldpc_dump_app<true>(decodeDesc,
                reinterpret_cast<const kernel_config_t::app_buf_t*>(smem_tb), iter,
                (max_info_nodes<kernel_config_t::BG>::value + 4) * decodeDesc.config.Z);
        }
    }
    else
    {
        ...
        iter = ldpc2::run_tb_iterations_with_early_termination<ENABLE_ACCESSORY_FEATURES, 1>(
            sched, decodeDesc, blockIdx.x,
            reinterpret_cast<const kernel_config_t::app_buf_t*>(smem_tb),
            smem_tb, size_before_et, crc);
    }

    ldpc_dec_output_variable_loop(decodeDesc, reinterpret_cast<const kernel_config_t::app_buf_t*>(smem_tb));
    if constexpr(!DUMP)
    {
    ...
    {
        ldpc_dec_soft_output(decodeDesc, reinterpret_cast<const kernel_config_t::app_buf_t*>(smem_tb));
    }
}

////////////////////////////////////////////////////////////////////////
// ldpc2_BG2_reg_box_plus_fp_tb_impl<ENABLE_ACCESSORY_FEATURES, DUMP>()
// Base graph 2 body, shared by _tb, _tb_no_accessories and _tb_dumpApp.
// DUMP adds the ET-free per-iteration APP dump; discarded when false.
template <bool ENABLE_ACCESSORY_FEATURES, bool DUMP>
__device__ __forceinline__
void ldpc2_BG2_reg_box_plus_fp_tb_impl(const cuphyLDPCDecodeDesc_t& decodeDesc, app_loc_t<2>::bg_desc_t bgdesc)
{
    // smem_tb, not smem: a template's extern __shared__ clashes (nvcc #1556)
    // with the non-template kernels in this file.
    extern __shared__ char smem_tb[];

    typedef ldpc2_reg_box_plus_fp_kernel_config<2,
                  NUM_STORAGE_WORDS_BG2,
                  MAX_NUM_PARITY_BG2,
                  cuphyLDPCDecodeConfigDesc_t> kernel_config_t;
    ...
    else
    {
        const uint32_t size_before_et = shmem_llr_buffer_size(
            ...
            decodeDesc.config.Z,
            sizeof(kernel_config_t::app_buf_t));
        iter = ldpc2::run_tb_iterations_with_early_termination<ENABLE_ACCESSORY_FEATURES, 2>(
            sched, decodeDesc, blockIdx.x,
            reinterpret_cast<const kernel_config_t::app_buf_t*>(smem_tb),
            smem_tb, size_before_et, crc);
    }

    ldpc_dec_output_variable(decodeDesc, reinterpret_cast<const kernel_config_t::app_buf_t*>(smem_tb));
    if constexpr(!DUMP)
    {
        ldpc2::write_early_termination_outputs<ENABLE_ACCESSORY_FEATURES, kernel_config_t::app_buf_t>(decodeDesc, blockIdx.x, crc, iter);
    }
...
////////////////////////////////////////////////////////////////////////
// ldpc2_BG1_reg_box_plus_fp_tb()
// Base graph 1 kernel, transport block interface
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG1_reg_box_plus_fp_tb(const __grid_constant__ cuphyLDPCDecodeDesc_t decodeDesc, app_loc_t<1>::bg_desc_t bgdesc)
{
    ldpc2_BG1_reg_box_plus_fp_tb_impl<true, false>(decodeDesc, bgdesc);
}

extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG1_reg_box_plus_fp_tb_no_accessories(const __grid_constant__ cuphyLDPCDecodeDesc_t decodeDesc, app_loc_t<1>::bg_desc_t bgdesc)
{
    ldpc2_BG1_reg_box_plus_fp_tb_impl<false, false>(decodeDesc, bgdesc);
}

////////////////////////////////////////////////////////////////////////
// ldpc2_BG2_reg_box_plus_fp_tb()
// Base graph 2 kernel, transport block interface
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG2_reg_box_plus_fp_tb(const __grid_constant__ cuphyLDPCDecodeDesc_t decodeDesc, app_loc_t<2>::bg_desc_t bgdesc)
{
    ldpc2_BG2_reg_box_plus_fp_tb_impl<true, false>(decodeDesc, bgdesc);
}

extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG2_reg_box_plus_fp_tb_no_accessories(const __grid_constant__ cuphyLDPCDecodeDesc_t decodeDesc, app_loc_t<2>::bg_desc_t bgdesc)
{
    ldpc2_BG2_reg_box_plus_fp_tb_impl<false, false>(decodeDesc, bgdesc);
}

#if CUPHY_LDPC_SPLIT_DUMP_KERNELS
////////////////////////////////////////////////////////////////////////
// ET-free per-iteration APP dump path (split-dump twins). The ET production
// kernels above are never touched: these are separate __global__ entry points,
// reached only when the caller sets CUPHY_LDPC_DECODE_DUMP_INTERM. Dumps are
// never timed, so one twin per base graph suffices and the timed kernels stay
// dump-free in every build.
////////////////////////////////////////////////////////////////////////
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG1_reg_box_plus_fp_tb_dumpApp(const __grid_constant__ cuphyLDPCDecodeDesc_t decodeDesc, app_loc_t<1>::bg_desc_t bgdesc)
{
    ldpc2_BG1_reg_box_plus_fp_tb_impl<false, true>(decodeDesc, bgdesc);
}
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG2_reg_box_plus_fp_tb_dumpApp(const __grid_constant__ cuphyLDPCDecodeDesc_t decodeDesc, app_loc_t<2>::bg_desc_t bgdesc)
{
    ldpc2_BG2_reg_box_plus_fp_tb_impl<false, true>(decodeDesc, bgdesc);
}
#endif // CUPHY_LDPC_SPLIT_DUMP_KERNELS
...
////////////////////////////////////////////////////////////////////////
// reg_box_plus_fp::decode()
cuphyStatus_t reg_box_plus_fp::decode(ldpc::decoder&                     dec,
                                      LDPC_output_t&                     tDst,
                                      ...
                                      cudaStream_t                       strm)
{
    DEBUG_PRINTF("ldpc2::reg_box_plus_fp::decode()\n");
    //------------------------------------------------------------------
    ...
    const uint32_t SHMEM_SIZE = shmem_size_with_et_context(shmem_llr_buffer_size(params.num_var_nodes, // num shared memory nodes
        ...
        case 1:
            {
                ...
                //-  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -
                // Launch the kernel
                ldpc2_BG1_reg_box_plus_fp<<<grdDim, blkDim, SHMEM_SIZE, strm>>>(params, *bgdesc);
        ...
        case 2:
            {
                ...
                //-  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -
                // Launch the kernel
                ldpc2_BG2_reg_box_plus_fp<<<grdDim, blkDim, SHMEM_SIZE, strm>>>(params, *bgdesc);
                s = CUPHY_STATUS_SUCCESS;
            }
            break;
        default:
...
////////////////////////////////////////////////////////////////////////
// reg_box_plus_fp::decode_tb()
cuphyStatus_t reg_box_plus_fp::decode_tb(ldpc::decoder&               dec,
                                         const cuphyLDPCDecodeDesc_t& decodeDesc,
                                         cudaStream_t                 strm)
{
    DEBUG_PRINTF("ldpc2::reg_box_plus_fp::decode_tb()\n");
        ...
        case 1:
            {
                ...
                const uint32_t SHMEM_SIZE = shmem_size_with_et_context(shmem_llr_buffer_size(decodeDesc.config.num_parity_nodes + max_info_nodes<1>::value, // num shared memory nodes
                ...
                {
                    // Dump requested: launch the ET-free dump twin.
                    DEBUG_PRINT_FUNC_MAX_BLOCKS(ldpc2_BG1_reg_box_plus_fp_tb_dumpApp, blkDim, SHMEM_SIZE);
                    ldpc2_BG1_reg_box_plus_fp_tb_dumpApp<<<grdDim, blkDim, SHMEM_SIZE, strm>>>(decodeDesc, *bgdesc);
                }
                else
#endif
                {
                    DEBUG_PRINT_FUNC_MAX_BLOCKS(ldpc2_BG1_reg_box_plus_fp_tb, blkDim, SHMEM_SIZE);
                    LDPC_LAUNCH_TB_KERNEL(ldpc2_BG1_reg_box_plus_fp_tb, decodeDesc.config.flags,
        ...
        case 2:
            {
                //------------------------------------------------------------------
                // Determine the dynamic amount of shared memory
                const uint32_t SHMEM_SIZE = shmem_size_with_et_context(shmem_llr_buffer_size(decodeDesc.config.num_parity_nodes + max_info_nodes<2>::value, // num shared memory nodes
                                                                  decodeDesc.config.Z,                                           // lifting size
                                                                  sizeof(__half)));                                              // element size
                ...
                {
                    // Dump requested: launch the ET-free dump twin.
                    DEBUG_PRINT_FUNC_MAX_BLOCKS(ldpc2_BG2_reg_box_plus_fp_tb_dumpApp, blkDim, SHMEM_SIZE);
                    ldpc2_BG2_reg_box_plus_fp_tb_dumpApp<<<grdDim, blkDim, SHMEM_SIZE, strm>>>(decodeDesc, *bgdesc);
                }
                else
#endif
                {
                    DEBUG_PRINT_FUNC_MAX_BLOCKS(ldpc2_BG2_reg_box_plus_fp_tb, blkDim, SHMEM_SIZE);
                    LDPC_LAUNCH_TB_KERNEL(ldpc2_BG2_reg_box_plus_fp_tb, decodeDesc.config.flags,
                        grdDim, blkDim, SHMEM_SIZE, strm, decodeDesc, *bgdesc);
                }
                s = CUPHY_STATUS_SUCCESS;
            }
...
////////////////////////////////////////////////////////////////////////
// reg_box_plus_fp::get_workspace_size()
std::pair<bool, size_t> reg_box_plus_fp::get_workspace_size(const ldpc::decoder&               dec,
                                                         const cuphyLDPCDecodeConfigDesc_t& config,
                                                         int                                num_cw)
{
...
////////////////////////////////////////////////////////////////////////
// reg_box_plus_fp::reg_box_plus_fp()
reg_box_plus_fp::reg_box_plus_fp(ldpc::decoder& desc)
{
    ...
    std::array<func_attr_t, 6 + (CUPHY_LDPC_SPLIT_DUMP_KERNELS ? 2 : 0)> func_attrs =
    {
        func_attr_t((const void*)ldpc2_BG1_reg_box_plus_fp,    MAX_BG1_SHMEM_SIZE),
        func_attr_t((const void*)ldpc2_BG2_reg_box_plus_fp,    MAX_BG2_SHMEM_SIZE),
        func_attr_t((const void*)ldpc2_BG1_reg_box_plus_fp_tb, MAX_BG1_SHMEM_SIZE),
        func_attr_t((const void*)ldpc2_BG1_reg_box_plus_fp_tb_no_accessories, MAX_BG1_SHMEM_SIZE),
        func_attr_t((const void*)ldpc2_BG2_reg_box_plus_fp_tb, MAX_BG2_SHMEM_SIZE),
        func_attr_t((const void*)ldpc2_BG2_reg_box_plus_fp_tb_no_accessories, MAX_BG2_SHMEM_SIZE)
#if CUPHY_LDPC_SPLIT_DUMP_KERNELS
       ,func_attr_t((const void*)ldpc2_BG1_reg_box_plus_fp_tb_dumpApp, MAX_BG1_SHMEM_SIZE)
       ,func_attr_t((const void*)ldpc2_BG2_reg_box_plus_fp_tb_dumpApp, MAX_BG2_SHMEM_SIZE)
#endif
    };
    ...
    //------------------------------------------------------------------
    DEBUG_PRINT_FUNC_ATTRIBUTES(ldpc2_BG1_reg_box_plus_fp);
    DEBUG_PRINT_FUNC_ATTRIBUTES(ldpc2_BG2_reg_box_plus_fp);
    DEBUG_PRINT_FUNC_ATTRIBUTES(ldpc2_BG1_reg_box_plus_fp_tb);
    DEBUG_PRINT_FUNC_ATTRIBUTES(ldpc2_BG2_reg_box_plus_fp_tb);
}

////////////////////////////////////////////////////////////////////////
// reg_box_plus_fp::get_launch_config()
cuphyStatus_t reg_box_plus_fp::get_launch_config(const ldpc::decoder&           dec,
                                                 cuphyLDPCDecodeLaunchConfig_t& launchConfig)
{
    ...
    cudaError_t    e = (BG == 1) ?  LDPC_GET_TB_KERNEL_FUNCTION(deviceFunction, ldpc2_BG1_reg_box_plus_fp_tb, launchConfig.decode_desc.config.flags):
                                    LDPC_GET_TB_KERNEL_FUNCTION(deviceFunction, ldpc2_BG2_reg_box_plus_fp_tb, launchConfig.decode_desc.config.flags);
#if CUPHY_LDPC_SPLIT_DUMP_KERNELS
    // Dump requested: select the ET-free dump twin.
    if(cudaSuccess == e && 0 != (launchConfig.decode_desc.config.flags & CUPHY_LDPC_DECODE_DUMP_INTERM))
    {
        e = (BG == 1) ? cudaGetFuncBySymbol(&deviceFunction, (void*)ldpc2_BG1_reg_box_plus_fp_tb_dumpApp)
                      : cudaGetFuncBySymbol(&deviceFunction, (void*)ldpc2_BG2_reg_box_plus_fp_tb_dumpApp);
    }
#endif
    if (e != cudaSuccess)
    {
